...
- **复执行证据**：`capability-hub.spec.ts` **3 passed**（2026-09-06，HEAD 54fd3dd，S10 Gate
  复执行轮；mock 模式对齐真实 Pydantic 投影，未模拟路径 fail loud）。
- **解锁条件核验**：被测前端能力已实现并入库（apps/web/src/features/capabilities/（S10-T4）+ capability-hub.spec.ts）， journey 覆盖与本条目

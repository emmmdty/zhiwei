...
- **复执行证据**：`ask-evidence.spec.ts` **4 passed**（2026-09-06，HEAD 54fd3dd，S10 Gate
  复执行轮；mock 模式对齐真实 Pydantic 投影，未模拟路径 fail loud）。
- **解锁条件核验**：被测前端能力已实现并入库（apps/web/src/renderers/ask/ + features/cases/（S10-T4b）+ ask-evidence.spec.ts）， journey 覆盖与本条目

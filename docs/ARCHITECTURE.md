...
- domain 不导入 FastAPI、Temporal、provider SDK 或前端 DTO。（2026-09-08 修订，F-R1-04：
  如实描述双层约定——**域状态机文件**（memory/domain.py、runtime/reducer.py、agents/task_graph.py
  等，登记于 tests/architecture/test_domain_state_machine_purity.py）不导入 SQLAlchemy/OpenSearch
  等 persistence/infra；persistence 以**模块内嵌 adapter** 形态存在于各域目录（identity/sessions.py、
  agents/claims.py、cases/pg_repository.py 等，§3 与 tests/architecture 登记清单为准），由同事务
  机制与 RLS 承载治理。）
...
src/zhiwei/
  ...
  memory/          records、policy、candidate、retrieval、forget、repositories（2026-09-04
                   ...
                   persistence，模块内嵌 adapter 为 repositories.py 与 events.py（canonical
                   event/audit 落账，S7 plan 授权；2026-09-08 F-R1-06 点名补记））

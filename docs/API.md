...
zhiwei release check|attest|render
...
### 12.1 ops 命令码表（S11-T5/T6 contract 冻结，plan F-R8-08）

`ops fault-run` / `ops load-run` / `backup create|verify` / `restore verify`：

| 码 | 语义 |
| --- | --- |
| 0 | 成功（全部场景/工作负载通过并按参数密封） |
| 1 | 执行失败（场景失败、校验失败、恢复失败——stdout 带失败明细） |
| 2 | 用法错误（未知 scenario/profile/workload、缺 `--isolated` 等参数面拒绝） |
| 3 | 环境不可用（docker/compose 栈未就绪——显式登记的环境阻塞，非静默跳过） |

`--help` 永远退出 0（CLI 既有惯例）。环境守卫 skip（pytest）与退出码 3 的区别：
测试进程用 skip（无 seal 产物），Gate 命令用 3（seal 记录环境阻塞，ADR-012 口径）。

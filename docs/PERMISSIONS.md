...
### 3.3 证据类 cell 的生产执行面登记（F-R3-06）

`authorize_mutation` 已支持 `resource_context` 证据通道（调用方从权威记录解析 SoD/own
当事人证据后传入；缺省为空时证据类动作在 PolicyInput 边界 fail closed）。各 cell 的
生产执行面状态登记如下（append-only，修复批次随时更新）：

| 矩阵 cell | 生产执行面 | 登记状态 |
| --- | --- | --- |
| tool_approval.approve/reject | runs 审批决策端点（requester/modifier 证据取自 approval 记录） | 已接线（T-P2a.1） |
| tool_approval.replace | 尚无 API 承载：runs 审批决策端点 decision 枚举仅 approved/rejected（无 replace 动作）；PEP resource_context 通道语义已恢复，承载面待后续阶段（P2b 返工核实——此前收口记录「与 approve/reject 同执行面」主张失实） | 待接线 |
| agent_publish.review_publish | releases advance 仍走 rollback cell：ReleaseRecord 无内容作者事实，权威证据源落地前维持降级 | 降级维持 |
| capability_version.review_high_critical | provider_action 高/关键风险仍走 admit_low_medium cell（P1 止血的 decision-only 面）；capabilities router 未在 P2b 重接范围，publisher 证据恢复随 S11 | 降级维持 |
| team_memory.confirm/correct/conflict/revoke | memory/confirm、/correct、/revoke、/delete、/conflicts/resolve 端点（T-P2b.1 接线；member 不再有 stub 期自确认/纠正/删除面——矩阵语义） | 已接线（P2b） |
| team_memory.read_authorized | memory 读路径（list/get/export/stats/conflicts）authorize_read（T-P2b.1） | 已接线（P2b） |
| connection_secret.create_workspace_connection | connections create（subject_mode=workspace_service）（T-P2b.1 接线） | 已接线（P2b） |
| connection_secret.revoke | connections actions revoke/suspend（suspend 无独立 cell，沿 revoke cell——P1 止血冻结映射）；own delegated 之外的全部 revoke 路径 | 已接线（P2b） |
| connection_secret.create_own / revoke_own | connections create（user_delegated，owner 证据=认证主体）/ actions revoke（own delegated，owner 证据取自权威记录）（T-P2b.1 接线）；service_account subject_mode 无矩阵 cell → 本地结构性拒绝 + denied 审计 | 已接线（P2b） |
| knowledge_source.manage | knowledge create/connect/sync/acl/disable 端点（矩阵行 4「创建/同步/授权/禁用」；T-P2b.1 接线） | 已接线（P2b） |
| org.read_self / team_memory.submit_own_candidate | 尚无 API 承载（own 语义 PEP 通道已恢复；承载面待后续阶段） | 待接线 |

**读路径执行面登记（P2b，append-only）**：

| 读面 | 矩阵 cell | 生产执行面 | 登记状态 |
| --- | --- | --- | --- |
| runs / evidence | run_case_artifact.read | runs 读端点 + evidence 端点（T-P2a.1；evidence 三通道随 T-P2b.2） | 已接线 |
| audit events | org.read_audit | GET /api/v1/audit-events（T-P2a.5） | 已接线 |
| team memory 读 | team_memory.read_authorized | memory 读路径（T-P2b.1） | 已接线 |
| connections 读（list/get/status） | 无 member 管理面 read cell（仅 auditor read_status_fingerprint） | 维持 P1 止血租户谓词（org+ws 过滤），无 PEP；S10 面板消费面承载待矩阵补充/裁决 | 登记（P2b） |
| knowledge 读（list/status/versions） | read={memory_steward}（「按 ACL 读来源」）、read_published={member} | 维持 P1 止血租户谓词，无 PEP：管理面 list 的 S10 消费方无 read cell 承载（workspace_admin 无 read cell 而有 manage cell——矩阵内部张力，待裁决）；read cell 的 ACL 语义消费面为 S7 检索（域级已接线） | 登记（P2b） |
| capability 读 | capability_version.read_admission_audit / browse | capabilities 面未在 P2b 重接范围（P1 止血 decision-only） | 登记（随 S11） |

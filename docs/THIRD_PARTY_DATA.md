...
| Temporal | durable execution | local compose：temporalio/server + admin-tools 同版本 pin（E-R1 方案 b）；dev server 由 SDK（uv.lock 锁定）默认版本承载；production external service | 已有资产；crash 场景已测（fault-run 13/13 sealed，2026-09-08） |
...
| `evals/novels` / `evals/risk` | 自建历史 FactQA/Risk 资产 | 由脚本生成并 asset lock | **发布复核完成（2026-09-08）**：novels docs 为本项目新创作改编文本（非原著引文，ReportLab 生成 PDF）、名著名录为公版古典名著事实数据、risk 为合成经营数据（gen_risk_data.py，无真实主体）——可再分发；随 `make evals`/`make determinism` 冻结 |

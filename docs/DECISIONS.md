...
> **2026-09-08 增补（F-R1-02 反例修订，D6 选 a 修文本）**：当前实现比本节字面更严——
> models/router.py 对 `unverified` 信任档**无条件硬拒**（不分数据分类），即路由层在
> classification ceiling 之外叠加了一道「未验证即拒」的门。该实现裁决被安全测试冻结
> （tests/security/model_egress/test_endpoint_trust.py）；方向更严非放宽，本增补承认该
> 硬拒门为 ADR-011 的现行语义。S11 生产 egress 接线时若需恢复「unverified + PUBLIC
> ceiling 放行」字面语义，须同步修订该冻结测试并走 ADR 修订流程。
...
## ADR-016：Claim 状态机直达 live 边（IMPLEMENTED → LIVE_VERIFIED，仅 live-production 作用域）

日期：2026-09-09　状态：已接受（operator 授权 live 演示任务；执行方按 ADR-012 反例程序修订）

### 反例（原状态机下 live claim 不可构造）

live-production 作用域的 claim 走 planned → implemented → offline_verified 时，
mix-rule（claims.py：live-production 口径拒绝一切非 live/shadow 密封件）使
offline_verified 中间步不可构造；而 (IMPLEMENTED, LIVE_VERIFIED) 证据边不存在
——live claim 无任何合法升级路径。S9 起仅 offline claim 实际存在，该缺口未暴露。

### 修订

1. `_EVIDENCE_EDGES` 新增 (IMPLEMENTED, LIVE_VERIFIED)：仅接受 live/shadow 密封件；
2. 该边仅限 `scope.environment == "live-production"` 的 claim——offline 作用域
   claim 仍须经 offline_verified 中间步（scope 纪律与 mix-rule 同源，不放宽）；
3. mix-rule 原语义不变：live-production claim 的 offline 证据仍然拒绝。

### 影响

- 冻结契约 tests/contract/release/test_claim_registry_frozen.py 新增
  TestLiveDirectEdge（3 条：live 直达 / shadow 直达 / offline 作用域仍拒跳级）；
  既有断言零改动（fixture_seal_cannot_verify_live_claim 在新边下语义保持）。
- 不影响既有 offline claim 的任何路径；retired 终态、bind_value 门禁不变。

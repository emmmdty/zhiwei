...
【降级映射，登记于 docs/PERMISSIONS.md §3.3】review_publish / tool_approval.approve
这两个语义上更精确的 cell 在 PolicyInput 边界强制 SoD 证据（last_content_author /
requester）。policy_gate.authorize_mutation 已支持 resource_context 证据通道
（F-R3-06，调用方从权威记录解析证据）；但 ReleaseRecord 不携带内容作者事实，
release 域内无可解析的 last_content_author 权威来源，在证据源落地前发布行治理
继续走 rollback cell；域层 require_transition_permission 的角色 SoD 不受影响
（双重防线中域层这道完整有效）。tool_approval.approve/reject 的接线随 runs 审批
决策端点改造落地（REMEDIATION_PLAN T-P2a.1，证据取自 approval 记录当事人）。

...
def create_memberships_router(
    *,
    ...
    async def list_org_members(
        ...
        # 读路径 PEP（ADR-012 决策 4；D-1 修复的 org 级半边）：matrix cell
        # org.read_memberships 只有 org_owner/security_admin（org 作用域）；
        # workspace_admin 须走 workspace 级端点（workspace 作用域绑定不匹配
        # org 上下文输入）。gate 以 actor 自身 org 构造——路径 org 与 actor org
        # 不一致时仍由 repository scope 守卫拒绝（读跨租户统一 404 防枚举）。
        ...
        # org 级资源读写的 RLS 上下文与被读资源对齐（workspace_id=None）：
        # 前端选中 workspace 后全局携带 ws 头，若沿用 actor 的 ws 声明，
        # repository 的 _require_org_level 守卫会把 org 级成员读写全拒成 404
        # （PEP gate 本就按 workspace_id=None 求值，ADR-014 对齐原则同款）。
        context = TenantContext(
            organization_id=actor.organization_id, workspace_id=None
        )

...
from zhiwei.policy.roles import Action, Purpose, ResourceType, Risk, RoleScope
...
def _actor_ref(actor: ActorContext) -> str:
    # S1 会话路径只产生 USER principal（repair addendum §3.1.11）；前缀随
    # principal.kind 走（F-R3-02/F-R2-13 残留：非 USER 被拒时审计主体失真）
    if actor.kind is PrincipalKind.USER:
        return f"user:{actor.principal_id}"
    return f"{actor.kind.value}:{actor.principal_id}"
...
def denied_audit_record(
    *,
    ...
    return AuditRecord(
        ...
        effective_identity_ref=_actor_ref(actor),
...
def _build_policy_input(
    *,
    ...
    risk 来自权威资源记录（PEP 职责：从存储记录解析输入），capability_version 的
    admit/review cell 依赖它做低中/高关键分流（缺失时 Rego fail closed 拒绝）。
    resource_context 与 risk 同一职责边界：调用方从权威记录解析 SoD/own 当事人证据
    （requester/last_content_author/publisher/owner）后传入；缺省为空 ResourceContext，
    证据类动作在 PolicyInput 边界 fail closed（F-R3-06 通道，语义由 Rego 侧裁决）。
    """
    role_bindings = tuple(
        binding_from_membership(
            binding.name,
    ...
    return PolicyInput(
        ...
        risk=risk,
        delegation=(),
        resource_context=resource_context if resource_context is not None else ResourceContext(),
...
async def authorize_mutation(
    *,
    ...
    - resource_context：SoD/own 当事人证据（F-R3-06 通道），调用方从权威记录解析后
      传入（如审批决策的 requester/modifier 来自 approval 记录）；缺省为空 → 证据类
      动作在 PolicyInput 边界 fail closed，与修复前行为一致。
    """
...
async def authorize_read(
    *,
    ...
    risk 语义与 authorize_mutation 相同（来自权威资源记录）。decision-only
    求值也用于 P1 止血窗口的 capability 生命周期动作角色裁决（mutation 审计
    随 P2b 纵切接线）——见 REMEDIATION_PLAN §3.P1/T-P1.2。
    """

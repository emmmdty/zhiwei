    ...
    *,
    ...
    async def create_user_endpoint(
        ...
        try:
            ...
            async with tenant_session(sessions, context) as session:
                # provisioning = org 归属（ADR-012 决策 4 的 membership 关联
                # 语义）：供给的 principal 与供给 org 建立 Membership（空
                # role_bindings = 无角色、仅组织归属）。归属与审计同一事务；
                # 但 create_user 是**先行的独立事务**——本块失败（瞬时 DB
                # 故障/非业务异常）会留下无归属 principal：跨 org 一律 404
                # （fail closed、非泄漏面），同 externalId 重试供给 409、
                # active=false 供给无法经 API 修复。provisioned 即成员的
                # 语义经 A2a 验收确认（验收记录 2026-09-07）；失败路径补偿
                # （解绑/删除 principal）归 S11。require_active=False：
                # principal 由本请求创建（liveness 自证），active=false 供给
                # （建后禁用）同样必须取得归属，否则 disable-at-creation 的
                # principal 对供给 org 永久不可见且无法 enable。
                await IdentityRepository(session, context).add_membership(
    ...
    async def get_user_endpoint(
        ...
        try:
            ...
            # ADR-012 决策 4 / F-R3-08：User 读按 org membership 关联过滤——
            # principal 是 identity-global 资源，membership 是 org 归属权威
            # 事实；跨 org principal 与不存在同文案 404（防枚举）。
            context = TenantContext(organization_id=actor.organization_id)
            async with tenant_session(sessions, context) as session:
                membership = await IdentityRepository(session, context).get_membership(
                    principal_id=principal_id, organization_id=actor.organization_id
                )
    ...
    async def _replace_or_patch_user(
        ...
        *,
        ...
        try:
            ...
            return _scim_response(exc)
        try:
            # F-R3-08：mutation 面同样按 membership 关联过滤——principal 是
            # identity-global 资源，跨 org 状态迁移（disable/enable）与读同
            # 一缺口；跨 org principal 与不存在同文案 404。
            context = TenantContext(organization_id=actor.organization_id)
            async with tenant_session(sessions, context) as session:
                membership = await IdentityRepository(session, context).get_membership(
                    principal_id=principal_id, organization_id=actor.organization_id

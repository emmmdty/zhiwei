...
def create_organizations_router(
    *,
    ...
    """组成时必须显式注入 actor dependency、业务 session 工厂与 policy_enforcer；
    缺少注入在构造期即失败（fail closed，policy_enforcer 缺失抛 TypeError）。
    membership 解析经 sessions（业务引擎）调用 resolver——membership 数据
    tenant-owned 在业务库（0027_rls_resolver 修订口径）。
    ...
    async def list_organizations(
        ...
        # 经业务引擎（sessions）调用：membership 数据 tenant-owned 在业务库，
        # 分库姿态下 identity 库只有 schema 副本（0027_rls_resolver 修订口径，
        # 与 sessions.memberships 同一批注）。
    ...
    async def bootstrap_organization(
        ...
        async with tenant_session(sessions, context) as session:
            try:
                ...
                # org。异常退出 tenant_session 后事务整体回滚（含刚插入的 org）；
                # loser target 不存在，无合法 audit FK scope → 沿用冻结的 pre-tenant
                # 例外：不写 failed 审计、零 audit/outbox（不得静默扩大该例外）。
                raise HTTPException(

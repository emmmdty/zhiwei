...
统一走 agent_publish.request（builder 提交/补交密封证据）。policy_gate 的
resource_context 证据通道已恢复（F-R3-06），但 review_publish 的 last_content_author
SoD 语义面向「复核发布」动作；claim register/upgrade 是提交侧动作，request cell
即矩阵本义，非降级。真正的升级门禁是服务层从 object store 独立复算密封件 + 域层

...
class TransitionManifest(BaseModel):
    ...
    # spec s3 §4（MODELS.md §5.1）：任何切换都使 provider prompt cache 完全
    # 失效——记录切换事实与预估重建成本，UI 切换前展示、成本进 ROI 指标。
    # F-R5-03：二者曾被 commit 接受后静默丢弃。
    cache_invalidated: bool = True
    rebuild_cost_estimate: dict[str, Any] | None = None
    # F-R6-08：做出切换判定所用 context 分类输入（审计可回答「当时按什么
    # 分类放行」）；epoch 路径无分类语境 → None（诚实缺省）

...
from zhiwei.evals.live_synthesis_suites import LIVE_SYNTHESIS_V1, resolve_live_synthesis_suite
...
app = typer.Typer(
    help="评测执行与密封（fixture/offline 面；live 面 live-gated，operator 显式触发）",
...
_KNOWN_SUITES: frozenset[str] = frozenset(
    {"legacy-assets", "runtime-contract-v1", LIVE_SYNTHESIS_V1}
...
# live-gated suite：mode=live + 显式 operator token 才能执行（live 触发是
# operator 显式动作，不是配置项）；fixture/offline 运行没有语义，一律拒绝。
# 仅 live 模型面 suite：S6–S10 行为契约 suite 的 offline/fixture 执行是 specs
# 冻结 Gate 口径（specs/s6 §Gate、s7 §8、s8 §9、s10 §4），不得纳入本集。
_LIVE_GATED_SUITES: frozenset[str] = frozenset({LIVE_SYNTHESIS_V1})
...
def run(
    ...
    operator_token: Annotated[
        ...
        typer.Option(
            "--operator-token",
            help="live 模式必需：operator 显式触发令牌（非空；不入库不落日志）",
        ),
    ] = None,
) -> None:
    """执行 suite 的全部注册单位（offline/fixture 面与 operator 门禁的 live 面）。"""
    if suite in _LIVE_GATED_SUITES:
        if mode is not EvalMode.LIVE:
            _fail(f"suite {suite} 是 live-gated：仅 --mode live 可执行（fail closed）")
        if operator_token is None or not operator_token.strip():
            _fail(
                "live 执行需要非空 --operator-token（live 只由 operator 显式触发，"
                "AGENTS.md 纪律）；令牌不落库不落日志"
            )
...
_LIVE_SYNTHESIS_GRAPH: dict[str, object] = {
    "nodes": {
        "live-synthesis": {
            "task_id": "live-synthesis",
            "task_type": "LiveSynthesis",
            "dependencies": [],
            "parallel_safe": False,
            "required_capability": "live-synthesis",
        }
    },
    "edges": {},
}
...
async def _provision_live_synthesis_run(
    sessions: Any, context: TenantContext, sample_id: str
) -> UUID:
    """可答单位的真实 Run 供给：生产命令路径（Run 行 + outbox StartRun 同事务）。

    canonical 落账（first use / wire manifest）经 CanonicalUnitOfWork 要求
    run_id 在租户作用域有真实 Run 行（RunNotFound fail closed）——live 合成
    因此落在生产 run 语义内，run id 成为可核验证据锚点。run id 用 uuid4：
    runs.id 是全局 PK，suite 级 deterministic_uuid 派生不含租户作用域，跨
    eval run（每次新租户）会同值碰撞——ask executor / faults 的生产命令
    路径同款随机 id。Run 行停留 created 态：不 dispatch workflow，本 suite
    的执行语义就是 evaluator 直连生产门禁链。
    """
    ...
    async with tenant_session(sessions, context) as session:
        ...
        await service.submit_start_run(
            run_id=run_id,
            graph=dict(_LIVE_SYNTHESIS_GRAPH),
            task_queue=DEFAULT_TASK_QUEUE,
        )
    return run_id


async def _live_synthesis_flow(
    sessions: Any,
    context: TenantContext,
    store: PosixObjectStore,
    *,
    ...
    """live 合成 suite：生产检索 + 真实模型合成（operator 门禁）+ 确定性判分。

    环境来源是显式注入的进程环境（operator 触发时 export 的 OPENAI_*；本函数
    不读 .env，AGENTS.md 纪律）。首次使用留痕与 wire manifest 走 canonical
    落账（CanonicalEndpointFirstUseSink / CanonicalManifestSink）。
    """
    import os
    ...
    endpoints_path = REPO_ROOT / "config" / "providers" / "endpoints.yaml"

    async def provision_run(sample_id: str) -> UUID:
        return await _provision_live_synthesis_run(sessions, context, sample_id)

    executor = LiveSynthesisExecutor(
        suite,
    ...
    result: dict[str, Any] = {
        ...
        "registered_units": len(suite.registered_units),
        "terminal_units": sum(
            1 for outcome in outcomes if outcome.status.value in {"completed", "failed", "refused", "error"}
        ),
        "status_counts": status_counts,
        "eval_run_id": str(created.eval_run_id),
        "organization_id": str(context.organization_id),
        ...
        async with _tenant_session(sessions, context) as session:
            ...
            sealed = await service.seal(
                ...
                test_report={
                    ...
                    "command": (
                        f"zhiwei eval run --suite {suite.name} "
                        f"--mode {mode.value} --seal --operator-token ***"
                    ),
                    "status_counts": status_counts,
                },
            )

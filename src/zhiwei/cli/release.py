...
from zhiwei.release.templates import RenderRefused, render_claim_surface
...
DEFAULT_RENDER_PATHS = (Path("README.md"),)
...
def render(
    paths: Annotated[
        list[Path] | None,
        typer.Option("--paths", help="待渲染的单一 release 表面文件（默认 README.md）"),
    ] = None,
    output: Annotated[
        Path | None,
        typer.Option("--output", help="渲染结果写出路径（默认 stdout；源文件永不回写）"),
    ] = None,
    db_dsn: Annotated[
        str | None, typer.Option("--db-dsn", help="maintenance DSN（默认取 ZHIWEI_DATABASE_URL）")
    ] = None,
) -> None:
    """渲染声明块 {{claim:ID}} → registry 绑定值（只接受 artifact-verified）。

    与 check 共享同一 maintenance-DSN 语义：README 对外呈现真实值，repo 内
    marker 永不回写（checker 的事实源）。拒绝路径上不产出任何输出。
    """
    surface = tuple(paths) if paths else DEFAULT_RENDER_PATHS
    if len(surface) != 1:
        _fail(f"render 每次只接受一个表面文件，收到 {len(surface)} 个")
    raw_dsn = db_dsn
    if raw_dsn is None:
        settings = _load_settings()
        if settings.database_url is None:
    ...
    try:
        ...
        _fail(f"无法读取 release 表面 {source.as_posix()}: {exc}")
    try:
        rendered = render_claim_surface(text, registry)
    except RenderRefused as exc:
        _fail(f"渲染被拒（表面保持原样）: {exc}")
    if output is None:
    ...
    try:
        ...
        _fail(f"无法写出渲染结果 {output.as_posix()}: {exc}")

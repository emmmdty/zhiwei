...
# E-R1 版本对齐（方案 b，2026-09-08 operator 确认）：dev server 不再显式 pin，
# 跟随 SDK（uv.lock 锁定 temporalio==1.32.0）的默认 dev server——实证 Server
# 1.31.2（GetSystemInfo.server_version）。compose/K8s 的 temporal 服务 pin
# temporalio/server:1.31.2 + admin-tools:1.31.2 schema setup 与该值精确对齐；
# 版本变更的唯一入口是 uv.lock 中 temporalio 的锁定版本。

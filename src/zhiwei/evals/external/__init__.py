...
ADR-013 决策 2、S7 plan Task 7（src/zhiwei/evals/external/）。
...
- 可用性探测是**确定性的本地检查**：许可文件 / version 文件 / 数据目录 / 数据文件的
  存在性。仓库当前没有任何 LongMemEval 数据 → 探测结果为 unavailable，并给出
  机器可读的缺失原因（缺什么、期望路径在哪）。
- available 分支「实际运行 adapter」的离线语义是**完整性执行**（corpus-integrity）：
  ...
  LongMemEval 质量 claim 都保持 `planned/unavailable`，不得用内部 fixture 替代
  外部诊断或写成已验证。
"""

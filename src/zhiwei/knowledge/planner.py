...
class KnowledgeTenantMismatchError(KnowledgePlannerError):
    """候选版本归属与查询租户不一致（F-R3-07 守卫，fail closed）。

    SourceVersion 契约不带归属（归属在 SourceObject 上）；预取层提供
    version_ownership 映射后，planner 对跨租户版本拒绝产出任何候选——
    跨租户内容不得以候选形态（含 locator/uri/digest）流向下游。
    """
...
class KnowledgePlanner:
    ...
    def generate_candidates(
        ...
        version_ownership: Mapping[UUID, tuple[UUID, UUID]] | None = None,
    ) -> list[QueryCandidate]:
        """Generate scored candidates from SourceVersions.

        1. Tenant guard (F-R3-07)：version_ownership 提供时，任一版本归属与
           query org/ws 不一致、或映射缺失条目 → KnowledgeTenantMismatchError
           （fail closed，一个候选不产出）。缺省 None 时守卫不启用——冻结 eval
           语料的跨 org 场景由 ACL 语义承载（allowed_groups 物化为目标 org），
           守卫生效会改变冻结评测结论；生产预取接线（S11）必须提供归属映射
           （登记约束，非可选优化）。归属元组与 SourceObject 契约同形
           (organization_id, workspace_id)，均为非空 UUID。
        2. Pre-filter by current ACL（守卫是第二道防线，不替代 ACL）。
        3. Score each candidate。
        4. Re-check ACL after hydration (ADR-006)。
        """
        if version_ownership is not None:
            self._guard_tenant_scope(query, versions, version_ownership)
    ...
    def _guard_tenant_scope(
        self,
        query: KnowledgeQuery,
        versions: list[SourceVersion],
        version_ownership: Mapping[UUID, tuple[UUID, UUID]],
    ) -> None:
        """(query.org/ws) vs (version 归属) 一致性守卫（F-R3-07，fail closed）。

        映射缺失条目同样拒绝：调用方声明提供归属却不完整，属于预取接线 bug，
        静默跳过即 fail open。归属元组与 SourceObject 同形，workspace_id 非空
        （contracts.py SourceObject 契约），org/ws 双谓词都必须命中。
        """
        for version in versions:
            ownership = version_ownership.get(version.id)
            if ownership is None:
                raise KnowledgeTenantMismatchError(
                    f"source version {version.id} has no ownership entry; "
                    "version_ownership mapping must be complete"
                )
            owner_org, owner_ws = ownership
            if owner_org != query.organization_id:
                raise KnowledgeTenantMismatchError(
                    f"source version {version.id} belongs to org {owner_org}, "
                    f"query targets org {query.organization_id}"
                )
            if owner_ws != query.workspace_id:
                raise KnowledgeTenantMismatchError(
                    f"source version {version.id} belongs to workspace {owner_ws}, "
                    f"query targets workspace {query.workspace_id}"
                )

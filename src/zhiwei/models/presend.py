...
class WireBodyTooLarge(PreSendRejected):
    """wire body 超过 ModelProfile.max_wire_body_bytes（F-R2-12 / ADR-001 结构约束 7）。

    body 一旦 materialize 就全量驻留内存；超限在发送前拒绝，而不是发出去再说。
    """
...
def wire_size_gate(max_wire_body_bytes: int) -> GateFn:
    """wire body 长度上界 gate（F-R2-12）：超限抛 WireBodyTooLarge，请求未离开进程。

    上界来源是 ModelProfile.max_wire_body_bytes 的声明值；由 client 工厂注入，
    与 classification gate 同为必选门禁。
    """
    limit = int(max_wire_body_bytes)
    if limit <= 0:
        raise ValueError("max_wire_body_bytes must be positive")

    def gate(capture: WireCapture, body: bytes) -> None:
        if len(body) > limit:
            raise WireBodyTooLarge(
                f"wire body {len(body)} bytes exceeds profile limit {limit} bytes"
            )

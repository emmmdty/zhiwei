...
from zhiwei.api.capabilities import _store as capability_hub
...
def create_app(
    ...
    *,
    ...
    session_service = SessionService(
        ...
        # membership resolver 读业务库的 tenant-owned 数据（分库姿态下 identity
        # 库只有 schema 副本，见 0027_rls_resolver 与 sessions.memberships 注释）
        membership_session_factory=app_sessions,
    )
    ...
    # S5 Knowledge / S7 Memory Center）。knowledge 自 P2b 起接 PG + PEP
    # （knowledge_source.manage cell）+ 同事务审计，disable 级联 ledger 失权
    # （F-R6-03/04）；connections 自 P2b 起接 PG + PEP（connection_secret
    # cells）+ 同事务审计；provider_version_id 存在性经 capability 目录查询
    # （组合根绑定，capability 持久层未建——P2b 范围外）。memory 自 P2b 起接
    # PG + PEP（team_memory cells）+ 同事务审计（F-R6-02/03）。capabilities 自
    # F-R9-01 止血起为供应链信任根，生命周期动作经 policy_enforcer 角色裁决
    # （决策式 PEP）。
    app.include_router(
        create_capabilities_router(
            actor_dependency=session_actor, policy_enforcer=policy_enforcer
        )
    )
    app.include_router(
        create_connections_router(
            ...
            sessions=app_sessions,
            policy_enforcer=policy_enforcer,
            provider_version_exists=lambda version_id, organization_id, workspace_id: (
                capability_hub.get_cap_version(
                    version_id,
    ...
    # S10 Studio draft 面的生产挂载（S11 followup-2 任务三实测发现缺失——
    # router 此前只有 re-export，组合根从未 include，GET /api/v1/agents 404，
    # Studio 停留 loading；mock e2e 经 context.route 拦截未经过组合根故未暴露）。
    app.include_router(
    ...
    # F-R6-09：审计事件列表（org.read_audit + RLS + 游标分页）——Auditor
    # journey 的产品内可达面（S1 §4）。
    app.include_router(
        create_audit_router(
            actor_dependency=session_actor,
            sessions=app_sessions,
    ...
    if settings.temporal_target is not None:
        ...
        app.include_router(
            create_runs_router(
                ...
                # F-R3-02/F-R2-13：runs 读与审批决策经 PEP（run_case_artifact.read /
                # tool_approval.approve・reject），决策审计同事务落账。
                policy_enforcer=policy_enforcer,
            )
        )
    ...
    # S11 local-product：healthcheck 探针端点（compose healthcheck / proxy 探活唯一消费方）。
    # 元数据-only：不触库、不做租户语义、不泄露拓扑细节——认证面之外唯一的公开路由，
    # 响应体只有 release_mode/profile 两个部署声明。
    @app.get("/healthz", include_in_schema=False)
    async def healthz() -> dict[str, str]:
        return {"release_mode": settings.release_mode.value, "profile": settings.profile.value}

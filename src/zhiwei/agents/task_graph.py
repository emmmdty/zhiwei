        ...
        for i, a in enumerate(task_ids):
            ...
            for b in task_ids[i + 1:]:
                ...
                for field in shared:
                    ...
                    a_strategy = self.nodes[a].output_merge_strategies[field]
                    b_strategy = self.nodes[b].output_merge_strategies[field]
                    if a_strategy != b_strategy:
                        # 增补 2「与字段一致的 merge 策略」：异构声明使运行时
                        # 合并语义无定义（如 APPEND×LWW 混合），发布期拒绝。
                        raise ValueError(
                            f"Parallel tasks '{a}' and '{b}' declare "
                            f"inconsistent merge strategies for field "
                            f"'{field}' ({a_strategy.value} vs {b_strategy.value}); "
                            f"every potential writer must declare a consistent "
                            f"merge strategy"
                        )

...
# (IMPLEMENTED, LIVE_VERIFIED) 为 ADR-016 修订：live-production 作用域 claim
# 无法经 offline_verified 中间步（mix-rule 拒绝 offline 证据）——live claim
# 以 live/shadow 密封件直达 live_verified；mix-rule 在边处理内保持原语义。
_EVIDENCE_EDGES: Mapping[tuple[ClaimStatus, ClaimStatus], frozenset[str]] = {
    ...
    (ClaimStatus.IMPLEMENTED, ClaimStatus.LIVE_VERIFIED): _LIVE_MODES,
...
def upgrade_claim(
    ...
    *,
    ...
    # ADR-016：直达 live 边仅限 live-production 作用域（offline 作用域 claim
    # 仍须经 offline_verified 中间步——scope 纪律与 mix-rule 同源）
    if (
        record.status is ClaimStatus.IMPLEMENTED
        and target is ClaimStatus.LIVE_VERIFIED
        and record.scope.environment != "live-production"
    ):
        raise ClaimUpgradeDenied(
            "direct live upgrade requires live-production scope; "
            "offline-scoped claims upgrade via offline_verified"
        )

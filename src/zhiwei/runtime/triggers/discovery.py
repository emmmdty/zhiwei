...
class TriggerStateStore(Protocol):
    """F-R5-05：trigger 状态持久 seam（watermark 推进 + delivery nonce）。

    生产实现 = PgTriggerStateStore（0023）；测试可注入内存实现。
    """
...
class DiscoveryTriggerService:
    ...
    async def source_delta_changed(
        self,
        program: DiscoveryProgram,
        version: ProgramVersion,
        trigger: Trigger,
        *,
        ...
        watermark 值变化才再次为 True。注入 state_store 时 watermark 持久
        （重启/多进程不丢——F-R5-05 缺口关闭）；未注入时退化为进程内 dict
        （仅测试形态，生产部署必须注入——重启即失，重复触发）。
        """
        DiscoveryTriggerService._validate(program, version, trigger)
        if trigger.type != "source_delta":
            raise TriggerFireError(f"trigger {trigger.type} is not a source delta")
        key = self._watermark_key(program, version, trigger)
        observed_value = json.dumps(observed, sort_keys=True, default=str)
        if self._state_store is not None:
            previous = await self._state_store.load_watermark(key)
            await self._state_store.store_watermark(key, observed_value)
        else:
            previous = self._watermarks.get(key)
            self._watermarks[key] = observed_value
        return previous is None or previous != observed_value

    async def verify_webhook_delivery(
        self,
        trigger: Trigger,
        *,
        delivery_id: str,
        ...
        """webhook 防重放（F-R5-05）：时间窗 + delivery nonce 一次性 claim。

        fail closed：未注入 state_store 直接拒绝（不退化为无防重放形态）；
        时间窗外（过期/超前重放）拒绝；同 delivery_id 重复投递拒绝。
        webhook HTTP 端点（S11 compose）必须先经本方法再 fire。
        """
        ...
        if self._state_store is None:
            raise TriggerFireError(
                "webhook delivery state store not configured; refusing to verify"
            )
        ...
        if not (now - skew <= timestamp <= now + skew):
            raise TriggerFireError(
                "webhook delivery timestamp outside replay window"
            )
        claimed = await self._state_store.claim_delivery(
            delivery_id, now.isoformat()
        )
        if not claimed:
            raise TriggerFireError("webhook delivery replay rejected")

    @staticmethod
    def _watermark_key(
        program: DiscoveryProgram, version: ProgramVersion, trigger: Trigger
    ) -> str:
        """跨进程稳定的 watermark 键：程序/版本/触发器声明共同决定身份。"""
        identity = f"{program.id}:{version.id}:{trigger.model_dump_json()}"

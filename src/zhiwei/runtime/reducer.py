...
class RunState(BaseModel):
    ...
    # Internal: per APPEND field, writer keys (task_id, attempt_no) per element,
    # parallel to the canonical list — 保序合并与幂等去重的身份面（ADR-005 增补 1）
    append_writers: dict[str, tuple[tuple[str, int], ...]] = Field(
...
def _apply_merge_field(
    ...
    append_writers: dict[str, tuple[tuple[str, int], ...]] | None = None,
    ...
    append_writers tracks per-element writer keys for APPEND fields（保序合并
    与幂等去重的身份面）；None 时仅排序生效、去重跨调用失效（白盒测试外的
    调用方必须传共享 dict）。
    """
    ...
    if strategy == MergeStrategy.APPEND:
        ...
        if not isinstance(existing, list):
            existing = [existing]
        # 首写登记 owner：后续未声明写入的降级 CP 路径据此把追加列表识别为
        # 已合并值（禁止静默接管，ADR-005 增补 3）；冲突记录以首写者键持
        # 先前合并值。若不登记，CP 分支 prev_task=None fallback 会整体覆盖
        # 追加列表且零冲突记录（对抗审查 F-R1-01-R）。
        if field not in field_owners:
            field_owners[field] = task_id
        writer_key = (task_id, attempt_no if attempt_no is not None else 0)
        prior_writers = () if append_writers is None else append_writers.get(field, ())
        if writer_key in prior_writers:
            # ADR-005 增补 1：同一 (task_id, attempt_no) 的重复投递按幂等去重
            return existing, None
        added = value if isinstance(value, list) else [value]
        writers = list(prior_writers)
        if len(existing) > len(writers):
            # 非 APPEND 路径先写的元素（标量接管形态）无写者身份——前置哨兵键
            writers = [("", -1)] * (len(existing) - len(writers)) + writers
        entries = [*existing, *added]
        writers = [*writers, *([writer_key] * len(added))]
        # 增补 1：合并序列由写者 (task_id, attempt_no) 决定——全量重排使
        # 结果成为写者集合的纯函数，与到达/完成序无关
        ordered = sorted(zip(entries, writers, strict=True), key=lambda pair: pair[1])
        if append_writers is not None:
            append_writers[field] = tuple(w for _, w in ordered)
        return [entry for entry, _ in ordered], None
    ...
    if strategy == MergeStrategy.LAST_WRITE_WINS:
        ...
        if prev is None:
            ...
            field_owners[field] = task_id
            if append_writers is not None:
                append_writers.pop(field, None)
            return value, None
        # Task-id ordering: lower task_id wins
        prev_task = field_owners.get(field)
        if prev_task is None or task_id < prev_task:
            field_owners[field] = task_id
            # LWW 替换使 APPEND 写者注册表失效（否则 canonical 与注册表
            # 失同步：收缩方向 zip 崩溃、等长替换静默保留陈旧键——B2 审查
            # MAJOR）。后续 APPEND 以哨兵键接续。
            if append_writers is not None:
                append_writers.pop(field, None)
            return value, None
...
def _handle_event(state: RunState, event: RuntimeEvent) -> RunState:
    ...
    if isinstance(event, TaskCompleted):
        ...
        append_writers = dict(state.append_writers)
        current_attempt_id = task.current_attempt_id
        attempt_no = 0
        if current_attempt_id is not None:
            attempt = task.attempts.get(current_attempt_id)
        ...
        if state.graph and event.task_id in state.graph.nodes:
            ...
            for field, value in event.output_values.items():
                ...
                if strategy is None:
                    # ADR-005 增补 3：未声明策略的写入（未经发布校验的图，如
                    # Planner 产物）降级为 conflict_preserving——禁止静默覆盖
                    # 已合并值；单写者 first-write 语义与声明前一致。
                    strategy = MergeStrategy.CONFLICT_PRESERVING
        ...
        result = state.with_update(
            ...
            field_owners=field_owners, append_writers=append_writers,
        )
        return result

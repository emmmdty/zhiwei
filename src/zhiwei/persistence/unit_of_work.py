...
from zhiwei.context.opaque import SCRUB_KEYS, scrub_hidden_reasoning
...
class CanonicalUnitOfWork:
    ...
    def __init__(
        ...
        *,
        ...
        scrub_keys: tuple[str, ...] = SCRUB_KEYS,
    ) -> None:
        if context is None:
        ...
        # 键名清单部署期常量（F-R2-06）：组装根可收窄，请求期不可变。
    ...
    async def append_event(self, command: EventCommand) -> EventAppendResult:
        ...
        command = command.model_copy(
            update={"payload": scrub_hidden_reasoning(command.payload, self._scrub_keys)}
        )

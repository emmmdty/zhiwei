...
class OutboxMessage(Base):
    ...
    # 0025/0026 三段式（S11-T3）：崩溃窗口 #2 的「pending 超龄」判定锚点。
    # contract 后 NOT NULL + server_default（insert 时刻 + 300s）——旧代码 INSERT
    # 不写该列时由 default 兜底；表达式与 0026 内 ALTER 的存储形态一致
    # （metadata-match 契约：tests/integration/foundation/test_database.py）。
    dispatch_deadline: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        nullable=False,
        server_default=text("(now() + '00:05:00'::interval)"),
    )
...
class ConnectionRow(Base):
    """S4 Connection 持久层（0020）：api/connections.py 的租户作用域投影。

    subject_mode/status 词表以 CHECK 约束镜像域 enum（capabilities.connections）；
    provider_version_id 是 capability 目录引用（目录持久层未建，存在性由 API
    层经注入查询校验，不设外键）；status 生命周期可变（version 递增），无
    内容不可变列。RLS/授权见 0020 迁移。
    """
    ...
    __table_args__ = (
        ...
        CheckConstraint("version > 0", name="connections_version"),
        CheckConstraint("schema_version > 0", name="connections_schema_version"),
        ForeignKeyConstraint(
            ["organization_id", "workspace_id"],
            ["workspaces.organization_id", "workspaces.id"],
        ...
        Index("ix_connections_workspace", "organization_id", "workspace_id"),
        Index("ix_connections_principal", "principal_id"),
    )

    id: Mapped[UUID] = mapped_column(Uuid, primary_key=True)
    organization_id: Mapped[UUID] = mapped_column(Uuid, nullable=False)
    workspace_id: Mapped[UUID] = mapped_column(Uuid, nullable=False)
    provider_version_id: Mapped[UUID] = mapped_column(Uuid, nullable=False)
    subject_mode: Mapped[str] = mapped_column(String(32), nullable=False)
    status: Mapped[str] = mapped_column(String(16), nullable=False)
    principal_id: Mapped[UUID | None] = mapped_column(Uuid)
    metadata_: Mapped[dict[Any, Any]] = mapped_column(
...
class SourceObjectRow(Base):
    """S5 SourceObject 持久层（0021）：Source Ledger 的对象注册表。

    acl 是唯一可变列（当前 ACL 权威，ADR-006 失权投影复检依赖——PUT acl 更新）；
    其余列不可变（触发器 + 列级授权双层，0021 迁移）。classification 词表镜像
    knowledge.contracts.Classification。
    """
    ...
    __table_args__ = (
        ...
        CheckConstraint("schema_version > 0", name="source_objects_schema_version"),
        ForeignKeyConstraint(
            ["organization_id", "workspace_id"],
            ["workspaces.organization_id", "workspaces.id"],
        ...
        Index("ix_source_objects_workspace", "organization_id", "workspace_id"),
        UniqueConstraint(
            "organization_id",
            "workspace_id",
            "id",
    ...
    acl: Mapped[dict[Any, Any]] = mapped_column(JSON_VALUE, nullable=False)
...
class SourceVersionRow(Base):
    """S5 SourceVersion 持久层（0021）：不可变版本行（更新 = 新版本）。

    content_digest CHECK 钉 sha256 协议（contracts.py 同款校验的数据面镜像）；
    (org, ws, object, seq) 与 (org, ws, object, digest) 唯一索引 = 重复 digest
    拒绝（DuplicateVersionError）的数据面防线；仅 state/tombstone/updated_at
    可变（0021 触发器守护内容列）。
    """
    ...
    __table_args__ = (
        ...
        CheckConstraint("version_seq >= 1", name="source_versions_seq"),
        CheckConstraint("schema_version > 0", name="source_versions_schema_version"),
        ForeignKeyConstraint(
            ["organization_id", "workspace_id"],
            ["workspaces.organization_id", "workspaces.id"],
        ...
        Index("ix_source_versions_workspace", "organization_id", "workspace_id"),
        UniqueConstraint(
            "organization_id",
            "workspace_id",
            "source_object_id",
    ...
    locator: Mapped[dict[Any, Any]] = mapped_column(JSON_VALUE, nullable=False)
    content_digest: Mapped[str] = mapped_column(String(71), nullable=False)
    observed_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
...
class TriggerWatermarkRow(Base):
    """Trigger 状态持久层（0023，F-R5-05）：source_delta watermark 推进记录
    与 webhook delivery nonce（一次性 claim）共用一张 tenant-scoped 表。

    scope/state_key 身份列不可变；value/updated_at 列级 UPDATE（watermark
    推进 = UPSERT）。无 DELETE——nonce 不可被应用角色抹除。
    """

    __tablename__ = "trigger_watermarks"
    __table_args__ = (
        CheckConstraint("schema_version > 0", name="trigger_watermarks_schema_version"),
        ForeignKeyConstraint(
            ["organization_id", "workspace_id"],
            ["workspaces.organization_id", "workspaces.id"],
        ...
        Index("ix_trigger_watermarks_workspace", "organization_id", "workspace_id"),
    )

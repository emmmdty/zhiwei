...
# sha256:<64 lowercase hex>——与 context/manifests.py、migration CHECK 同一口径。
...
class InvocationFailureReason(StrEnum):
    ...
    CAPABILITY_DRIFTED = "capability_drifted"
...
class SandboxSpec(_FrozenModel):
    ...
    def validate_sandbox(self) -> list[str]:
        ...
        if not self.no_network:
            violations.append("network access must be disabled (no_network)")
        if not self.image_digest:
            violations.append("image digest must be pinned")
        elif _SHA256_DIGEST_PATTERN.fullmatch(self.image_digest) is None:
            violations.append(

...
class ToolGateway:
    ...
    def __init__(
        ...
        *,
        ...
        provider_registry: dict[UUID, ProviderVersion] | None = None,
    ) -> None:
        ...
        self._providers = provider_registry or {}
        ...
        *,
        ...
        with start_span(
            ...
            try:
                ...
                capability = self._validate_capability(invocation)
                ...
                invocation = await self._build_sandbox(invocation, capability)
    ...
    def _validate_capability(self, invocation: ToolInvocation) -> CapabilityVersion:
        """Validate capability version is published and not revoked.

        绑定 digest 复核（F-R9-03）：capability 钉住的 content_digest 与 provider
        当前内容 digest 经 detect_capability_drift 复核——准入后内容被篡改/上游
        换内容即拒绝执行（fail closed）。provider 未接线（registry 缺项）时跳过
        复核——组合根按装配决定复核面。
        """
        ...
        provider = self._providers.get(invocation.provider_version_id)
        if provider is not None:
            drift = detect_capability_drift(
                bound_content_digest=capability.content_digest,
                current_content_digest=provider.content_digest,
                bound_test_digest=capability.test_digest,
                current_test_digest="",
            )
            if not drift.passed:
                raise ToolGatewayError(
                    "Capability binding digest mismatch: upstream provider "
                    "content drifted after admission"
                )
    ...
    async def _build_sandbox(
        self, invocation: ToolInvocation, capability: CapabilityVersion
    ) -> ToolInvocation:
        """Build OCI sandbox specification for isolated execution.

        digest 链（F-R2-02/F-R9-04）：sandbox 的 image_digest 必须取自
        CapabilityVersion.content_digest（准入期钉住的 digest），构造期校验相等；
        未 pin 或畸形的 digest 一律拒绝（fail closed）——不再每次调用随机伪造。
        """
        pinned_digest = capability.content_digest
        if not pinned_digest:
            raise ToolGatewayError(
                "Capability content digest is not pinned; refusing sandbox build"
            )
        ...
        # 构造期校验相等：spec 携带的 digest 与 capability pin 完全一致（防御性，
        # 唯一构造路径下恒真；若未来引入第二构造点则在此拦截）。
        if sandbox.image_digest != pinned_digest:
            raise ToolGatewayError(
                "Sandbox digest does not match pinned capability content digest"
            )
    ...
    def _fail_invocation(
        ...
        *,
        ...
        elif "drift" in msg or "mismatch" in msg:
            reason = InvocationFailureReason.CAPABILITY_DRIFTED
        elif "sandbox" in msg or "digest" in msg:
            reason = InvocationFailureReason.SANDBOX_VIOLATION

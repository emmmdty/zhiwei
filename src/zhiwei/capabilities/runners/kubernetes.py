...
class KubernetesRunner(BaseRunner):
                ...
                "template": {
                    "spec": {
                        ...
                        "containers": [
                            {
                                "name": "tool-executor",
                                # 容器 image = 沙箱 pinned digest（capability
                                # content_digest）：准入钉住的 digest 才是实际
                                # pull 的对象（F-R9-04）。
                                "image": f"{sandbox['image_digest']}",
    ...
    def _validate_sandbox_k8s(self, sandbox: dict[str, Any]) -> list[str]:
        ...
        与域层 SandboxSpec.validate_sandbox 同一组约束（F-R2-04/F-R9-04）；
        Job 容器 image 取沙箱 pinned digest（admission 钉住的 digest 才是实际
        pull 的对象），故 digest 格式在 pull 前校验。
        """
        violations: list[str] = []
        ...
        if sandbox.get("read_only_rootfs") is False:
            violations.append("K8s container rootfs must be read-only")
        if sandbox.get("no_network") is False:
            violations.append("network access must be disabled for K8s pod")
        digest = sandbox.get("image_digest")
        if not digest:
            violations.append("K8s runner requires pinned image digest")
        elif _SHA256_DIGEST_PATTERN.fullmatch(str(digest)) is None:
            violations.append(
                "Image digest must be a lowercase sha256:<64 hex> digest"
            )
        return violations

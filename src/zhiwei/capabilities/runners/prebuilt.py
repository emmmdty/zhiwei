...
class PrebuiltRunner(BaseRunner):
    ...
    def _validate_sandbox(self, sandbox: dict[str, Any]) -> list[str]:
        ...
        与域层 SandboxSpec.validate_sandbox 同一组约束（F-R2-04）；digest 校验是
        pull 前的最后一道——格式合法的 pinned digest 才允许进入执行（字节级
        verify 随真实 runner 接入在 S11 落地）。
        """
        violations: list[str] = []
        ...
        if sandbox.get("read_only_rootfs") is False:
            violations.append("Container rootfs must be read-only")
        if sandbox.get("no_network") is False:
            violations.append("network access must be disabled for prebuilt sandbox")

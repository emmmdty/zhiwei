...
class ApprovalPEP:
    ...
    def execute_publish(
        ...
        *,
        version_manager: CapabilityVersionManager | None = None,
    ) -> ApprovalCommandResult:
        """Execute publish with full PEP validation.

        F-R9-06 修复：version_manager 提供时在同一调用内完成
        结构合法性预检（跳步 → InvalidTransitionError，先于审批评估）
        → readiness（审批不足/同主体/digest 失效 → success=False）
        → CAS transition（并发冲突 → VersionConflictError）。
        预检/转换异常原样上抛，由调用方映射 HTTP 语义；不再返回
        无操作的 success=True。version_manager=None 保留纯 readiness
        求值（既有调用方语义，不放宽）。
        """
        if version_manager is not None:
            version_manager.check_transition(version_id, CapabilityStatus.PUBLISHED)

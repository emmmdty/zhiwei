...
from zhiwei.models.classification import derive_context_classification
...
class ModelActivity:
    ...
    def _build_routing_request(
        ...
        F-R6-08：data_classification 不再缺省虚构 "public"——声明与 context
        inventory 派生并存取 max（派生顶起低报声明、过高声明保留）；二者皆无
        → fail closed 拒绝组装。
        """
        rr = input.routing_request
        declared = rr.get("data_classification")
        derived = derive_context_classification(rr.get("context_items", ()))
        if declared is None and derived is None:
            raise ValueError(
                "routing request carries no data_classification and context "
                "inventory declares none; refusing to fabricate 'public' (F-R6-08)"
            )
        if declared is None:
            # 仅派生（derived 非 None——上方守卫保证）
            effective = str(derived)
        elif derived is not None:
            effective = max(
                ClassificationCeiling(str(declared).lower()),
                ClassificationCeiling(derived),
            ).value
        else:
            # declared-only 同样过 enum 构造器（校验 + 归一）——裸传未知值
            # 到 router rank -1 会 fail-open 静默放行（审查 F1）
            effective = ClassificationCeiling(str(declared).lower()).value

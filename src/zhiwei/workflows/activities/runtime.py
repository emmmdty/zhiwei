...
class RuntimeActivities:
    ...
    async def _execute_task(self, input: ExecuteTaskInput) -> TaskExecutionResult:
        ...
        async with tenant_session(self._sessions, context) as session:
            ...
            if not await store.has_event(
                ...
                # 字段与未声明策略字段（增补 3 降级面）存在 prior 值时，
                # TaskCompleted 之外追加 ConflictDetected canonical event（重放后
                # 冲突证据仍在，不只存在于内存投影）。
                # prior 值/写者在追加前从当前事件序列 reduce 得出；reduce 必须
                # 在 run advisory lock 内——并发完成事务串行化后，后落账者才能
                # 确定性地看到先落账者的 TaskCompleted（读已提交语义）。
                # 落账面按实际输出判定：声明 APPEND/LWW 的字段不产生冲突事件。
                event_fields = tuple(
                    sorted(
                        field_name
                        for field_name in output.output_values
                        if field_name in input.conflict_fields
                        or field_name not in input.merge_strategies
                    )
                )
                state_before = None
                if event_fields:
        ...
        return TaskExecutionResult(
            ...
            status="completed",
            attempt_no=input.attempt_no,
            # Temporal history 是独立持久化面（F-R2-06/T-P3.7）：activity 返回值
            # 在出口 scrub——PG canonical 行的 UoW scrub 不覆盖 history。
            output_values=scrub_hidden_reasoning(dict(output.output_values)),
        )
    ...
    async def _append_conflict_events(
        ...
        """conflict_preserving 与未声明策略（降级 CP）字段的冲突落账
        （双方 task/value + attempt 证据）。"""
        run_id = UUID(input.run_id)
    ...
    @activity.defn
    async def run_intent_recheck(self, input: RunIntentRecheckInput) -> RunIntentRecheckResult:
        """CAN 边界的 PG 意图回查（S11-T5，崩溃窗口 #11 兜底）。

        只查 outbox 中 pending 的 cancel_run/pause_run 命令（payload.kind 命名与
        RunCommandService 写入一致）；不消费、不转移状态——信号的最终交付仍归
        dispatcher，回查只回答「有没有已落账但可能被 CAN 间隙吞掉的意图」。
        """
            ...
            rows = (
                await session.scalars(
                    select(OutboxMessage).where(
                        ...
                        OutboxMessage.event_key.in_(["cancel_run", "pause_run"]),
                    )
                )
            ).all()
            for row in rows:
                ...
                if str(payload.get("run_id", "")) != run_id_str:
                    continue
                if payload.get("kind") == "cancel_run":
                    cancel_pending = True
                    cancel_reason = payload.get("reason")
                elif payload.get("kind") == "pause_run":
                    pause_pending = True
        return RunIntentRecheckResult(

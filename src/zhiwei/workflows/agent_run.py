...
class AgentRunWorkflowInput:
    ...
    # F-R2-09：委托链（ADR-008 第②层断言输入）；CAN 边界必须存活。
...
class AgentRunWorkflow:
    ...
    async def run(self, input: AgentRunWorkflowInput) -> AgentRunWorkflowResult:
        ...
        while True:
            ...
            if (
                ...
                # 崩溃窗口 #11 兜底（S11-T5 RED 冻结）：CAN 间隙的信号仍可能
                # 丢失——过渡前再查一次 PG 意图（outbox pending 的 cancel/pause
                # 命令是已落账真相）。有 cancel 意图 → 本地记 cancelled 终态
                # （被丢弃的 signal 由 outbox 命令语义代表）；有 pause 意图 →
                # 置暂停态并留在本 run。两者都不出现才继续 CAN。
                intent = await workflow.execute_activity(
                    ...
                    retry_policy=_INFRA_RETRY_POLICY,
                    task_queue=input.task_queue,
                    result_type=RunIntentRecheckResult,
                )
                if intent.cancel_pending:
                    self._cancel_requested = True
                    self._cancel_reason = intent.cancel_reason or "cancel intent from outbox"
                    await self._record_terminal(input, "cancelled")
                    return self._result(input, "cancelled")
                if intent.pause_pending:
                    self._paused = True
    ...
    def _start_task(
        ...
        # 落账的判定输入）；完整策略映射同时下发——未声明策略的字段属 ADR-005
        # 增补 3 降级面，其冲突同样必须落账（增补 4）。

...
class SessionService:
    ...
    def __init__(
        ...
        *,
        ...
        membership_session_factory: async_sessionmaker[AsyncSession],
    ) -> None:
        self._session_store = session_store
        self._secret_backend = secret_backend
        ...
        # membership 数据是 tenant-owned（业务库）；resolver 函数随迁移流存在于
        # 两个库，但只有业务库有数据——分库姿态下必须经业务引擎调用（单库姿态
        # 两引擎同库，行为一致）
        self._membership_session_factory = membership_session_factory
    ...
    async def memberships(self, principal_id: UUID) -> list[dict[str, Any]]:
        ...
        经业务引擎调用：membership 数据 tenant-owned 在业务库（分库姿态下
        identity 库只有表的空 schema 副本）；单库姿态两引擎同库等价。
        """
        async with self._membership_session_factory() as session:
            result = await session.execute(
                text(

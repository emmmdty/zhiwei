...
class BootstrapClaimConflict(IdentityCommandError):
...
class ActorContext(_FrozenModel):
    ...
    active_organization_ids 由 resolve_context 从全部权威 memberships 解析：所有
    organization_status='active' 的绑定（org 作用域与 workspace 作用域都计）所属组织的
    id 集合，去重后稳定排序；作为 bootstrap / 重放候选判定输入，绝不信任 header/body。
    """

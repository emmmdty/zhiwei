...
class IdentityRepository:
    ...
    async def add_membership(
        ...
        *,
        ...
        require_active=False 仅限同一请求内刚创建 principal 的供给流（SCIM
        provisioning）：liveness 由创建动作自证，且 membership 在 disable 后
        存活本就是域语义（disable 不删 membership）——对 active=false 供给
        拒绝归属会让该 principal 对供给 org 永久不可见（ADR-012 决策 4 的
        membership 关联语义断裂）。
        """
        context = self._require_context()
        self._require_organization(organization_id, context)

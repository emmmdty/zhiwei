...
def redact_value(value: str) -> str:
    """F-R6-05（D6）：终态覆写占位——redacted 前缀 + 原内容 SHA-256。

    boundary 语义：原文不可恢复，digest 保留可验证性（PERMISSIONS「历史
    Run 只保留必要 digest/redacted marker」）；占位不含原文任何碎片。
    """
    return f"redacted:{digest(value)}"
            ...
            if self.retention.is_candidate_expired(record.created_at, now_utc):
                updated = record.model_copy(
                    update={
                        ...
                        # F-R6-05：终态覆写 digest 占位（TTL 过期同遗忘语义）
                        "canonical_value": redact_value(record.canonical_value),
                        "tombstone": True,
                        "updated_at": now_utc,
                    }
                )
        ...
        *,
        ...
        revoked = record.model_copy(
            update={
                ...
                # F-R6-05：终态覆写 digest 占位（redaction 与终态转移同一步）
                "canonical_value": redact_value(record.canonical_value),
                "tombstone": True,
                "updated_at": now_,
            }

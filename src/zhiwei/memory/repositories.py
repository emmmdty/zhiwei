...
class PgMemoryRepository:
    ...
    async def revoke_record(
        ...
        *,
        ...
        actor_ref 缺省沿用内部 writer 词表（memory:write:{author}）；API 发起的
        遗忘操作传入认证主体引用（F-R6-02：审计主体 = 发起人）。
        """
        context = self._require_context()
        await self._lock(context)
        existing = await self._find_active(context, _hash_of(dedup_key))
        ...
        await self._persist_transition(
            ...
            existing_canonical_value=existing.canonical_value,
        )
        await self._emit(
            revoked,
            ...
            actor_ref=actor_ref if actor_ref is not None else _writer_actor(revoked),
            reason=reason,
        )
        return revoked
    ...
    async def expire_candidates(
        ...
        for original, expired_record in zip(records, expired, strict=True):
            await self._persist_transition(
                ...
                existing_canonical_value=original.canonical_value,
            )
            await self._emit(
                expired_record,
    ...
    async def list_for_user(
        ...
        *,
        ...
        """Memory Center 可见记录（S7 §5）：USER scope 仅本人，team/case 全
        workspace 可见。可见性是数据访问谓词，与租户谓词同层落在仓储。"""
        return await self._list(
            visible_to_user=user_id, scope=scope, mem_type=mem_type, mem_status=mem_status
        )

    async def list_for_tenant(self) -> list[MemoryRecord]:
        """租户全量记录（无本人可见性过滤）——conflict 派生投影等 steward
        面使用；调用方（team_memory cell）已经 PEP 裁决。"""
        return await self._list()

    async def _list(
        self,
        *,
        visible_to_user: UUID | None = None,
        scope: MemoryScope | None = None,
        mem_type: MemoryType | None = None,
        mem_status: MemoryStatus | None = None,
    ) -> list[MemoryRecord]:
        ...
        if visible_to_user is not None:
            statement = statement.where(
                or_(
                    MemoryRecordRow.scope != MemoryScope.USER.value,
                    MemoryRecordRow.scope_subject_id == visible_to_user,
                )
            )
        if scope is not None:
            statement = statement.where(MemoryRecordRow.scope == scope.value)
        if mem_type is not None:
            statement = statement.where(MemoryRecordRow.type == mem_type.value)
        if mem_status is not None:
            statement = statement.where(MemoryRecordRow.status == mem_status.value)
        statement = statement.order_by(MemoryRecordRow.created_at, MemoryRecordRow.id)
        rows = list((await self._session.scalars(statement)).all())
    ...
    async def _persist_transition(
        ...
        *,
        ...
        existing_canonical_value: str | None = None,
    ) -> None:
        ...
        if existing_canonical_value is not None and (
            record.canonical_value != existing_canonical_value
        ):
            # F-R6-05（D6）：终态转移的 redaction 覆写与状态 CAS 同一条
            # UPDATE（0012 触发器要求：终态后无二次覆写窗口）。
            values["canonical_value"] = record.canonical_value
        statement = (

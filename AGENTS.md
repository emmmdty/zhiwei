...
Knowledge、Evidence/Ask、Memory、Discover/Actions）实现已入库并经整批验收
（docs/handoffs/s3-s8-batch-acceptance.md）；S9（Eval/Release/Observability）、S10（Studio/Third App）
已分别于 2026-09-06 Gate 收口（docs/handoffs/s9-eval-release-observability.md §5、
docs/handoffs/s10-studio-third-app.md §5）。未开始：S11 Production Reference。

事实源优先级（冲突时上位优先）：

1. `docs/superpowers/specs/2026-08-12-zhiwei-enterprise-agent-platform-design.md` — 冻结总设计（架构约束）
2. `docs/DECISIONS.md` — ADR-001~015（机制级算法与流程例外，总设计冻结后补齐）
...
```bash
make evals            # 重建并校验冻结基准资产（1205 项 / 32 个冻结资产）
...
- 域状态机文件不导入 FastAPI/Temporal/SQLAlchemy/provider SDK；persistence 以模块内嵌 adapter
  形态存在于各域目录（双层约定与登记清单见 `docs/ARCHITECTURE.md` §2/§3、
  `tests/architecture/test_domain_state_machine_purity.py`，F-R1-04 修订口径）。

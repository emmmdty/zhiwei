...
interface RunRecord {
  ...
  // planner 意图标识（runs.template，创建期持久化）——run 无自由命名机制，
  // template 是列表里区分「这是一次什么执行」的唯一人类可读锚点
        ...
        <table>
          ...
          <tbody>
            {runs.map((run) => (
              <tr key={run.run_id}>
                <td>{run.run_id}</td>
                {/* mock 面（accessibility.spec）的 run 载荷不带 template——
                    undefined 与 null 同样显示占位，不猜默认值 */}
                <td>{run.template ?? "—"}</td>
                <td>{run.status}</td>
                <td>

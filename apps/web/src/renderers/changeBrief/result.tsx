  ...
  return (
    <section aria-label="Verified brief">
      {!terminal ? (
        ...
        <>
          {/* E7（operator D7 采纳）：terminal 且 evidence 已取到而 brief 缺席
              ——「不会到来」（投影未携带）而非「还会到来」（in-flight），
              两态措辞必须可区分。 */}
          <StateBanner
            tone="empty"
            text={`Brief not included in the evidence projection yet (run ${run.runId}: ${run.status})`}
          />

...
function installStudioMocks(context: BrowserContext, state: MockState, actor: Actor): void {
  ...
  context.route("/api/**", async (route) => {
    ...
    // E4 契约收敛（F-R7-03/D7-a）：冻结矩阵下 auditor 无 agent_draft.read
    // （PERMISSIONS「读版本与 Gate」）——mock 与生产策略同口径，403。
    if (actor === "auditor" && path === "/api/v1/agents" && method === "GET") {
      return fulfill(route, 403, { detail: "not authorized (agent_draft.read denied)" });
    }
...
test.describe("S10 studio — role gating", () => {
  // E4 契约收敛（F-R7-03，operator D7-a：维持 deny、UI/e2e 修契约）——
  // 冻结矩阵下 auditor 无 agent_draft.read，真实旅程 = 403 诚实横幅；
  // 原「富只读面」断言与生产策略是两个旅程，按 403 面收敛。
  test("auditor gets honest 403 banner, draft surface unreachable", async ({ browser }) => {
    const state = newState();
    ...
    await page.goto("/");
    await page.getByRole("button", { name: "Agent Studio" }).click();
    await expect(page.getByText(/Not authorized for Agent Studio \(403\)\./)).toBeVisible();
    // 富只读编辑面不可达（draft 列表未渲染）

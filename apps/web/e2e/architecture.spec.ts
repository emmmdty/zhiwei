...
test.describe("S10 fix-B — verified brief rendering", () => {
  ...
  test("change brief run without a projected brief states absence, not in-flight (E7)", async ({ browser }) => {
    const state = newState();
    const context = await newContextWithMocks(browser, state);
    const page = await context.newPage();
    ...
    // 投影无 brief → absent（不会到来）而非 in-flight（还会到来）如实区分（E7）；Evidence 面板如实空态
    await expect(page.getByText(/Brief not included in the evidence projection yet/)).toBeVisible();

...
class TestBatchCRepairContracts:
    ...
    async def test_undeclared_parallel_conflict_lands_as_event(
        self, temporal_env, worker_stack
    ) -> None:
        """ADR-005 增补 3/4：未声明策略的并行写（未发布图）不得静默覆盖——
        降级 conflict_preserving 且 ConflictDetected 作为 canonical event 落账。"""
        from zhiwei.runtime.events import ConflictDetected
        from zhiwei.runtime.handlers.base import TaskHandler, TaskInput, TaskOutput
        from zhiwei.runtime.handlers.core import SynthesizeHandler

        class _UndeclaredFixtureHandler(TaskHandler):
            """输出共享字段但节点零策略声明（Planner 未发布图形态）。"""
        ...
        def _undeclared_node(task_id: str) -> TaskGraphNode:
            return TaskGraphNode(
                task_id=task_id,
                task_type="UndeclaredFixture",
                dependencies=(),
                parallel_safe=True,
                required_capability="fixture",
                output_schema={"decision": {"type": "string"}},
        ...
        graph = TaskGraph(
            nodes={
                "a": _undeclared_node("a"),
                "b": _undeclared_node("b"),
                "synth": TaskGraphNode(
                    task_id="synth",
                    task_type="Synthesize",
                    dependencies=("a", "b"),
        ...
        if not registry.has_handler("Synthesize"):
            registry.register(SynthesizeHandler())
        if not registry.has_handler("UndeclaredFixture"):
            registry.register(_UndeclaredFixtureHandler())
        run_id, handle = await self._start(temporal_env, sessions, context, graph)
        result = await asyncio.wait_for(handle.result(), timeout=60)
        assert result.status == "completed"
        ...
        types = [type(e).__name__ for e in events]
        assert "ConflictDetected" in types, (
            "未声明策略的并行写降级后冲突必须作为 canonical event 落账（ADR-005 增补 4）"
        )
        conflict_events = [e for e in events if isinstance(e, ConflictDetected)]
        assert any(c.field == "decision" for c in conflict_events)
        assert len(state.conflicts) >= 1
        decision_conflicts = [c for c in state.conflicts if c.field == "decision"]
        assert decision_conflicts, "降级冲突必须进入投影"
        merged_values: set[str] = set()
        for conflict in decision_conflicts:
            merged_values.update(str(v) for v in conflict.values.values())
        assert {"value-a", "value-b"} <= merged_values, (
            "双方值必须并存于 ConflictRecord（禁止静默覆盖，ADR-005 增补 3）"
        )
        synth_output = state.tasks["synth"].output_values
        assert synth_output.get("synthesize_downgraded") is True, (
            f"未声明并行写产生的冲突同样必须触发 Synthesize 降级（实际输出 {synth_output}）"
        )

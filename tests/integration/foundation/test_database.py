...
REQUIRED_TABLES = {
    ...
    # P2b（0020/0021）：connections + Source Ledger 持久层（F-R6-03/04；FORCE
    # RLS、org+ws 作用域）——REQUIRED_TABLES 契约随新表扩展（0011 同款登记协议）。
    "connections",
    "source_objects",
    "source_versions",
    # P4（0023）：trigger watermark/delivery nonce 持久层（F-R5-05；FORCE
    # RLS、org+ws 作用域）——REQUIRED_TABLES 契约随新表扩展（0011 同款登记）。
    "trigger_watermarks",
}
WORKSPACE_TABLES = {
    "agent_definitions",
    ...
    "source_objects",
    "source_versions",
    # P4（0023）：trigger state 行为 workspace 作用域租户数据（org+ws GUC RLS）
    "trigger_watermarks",
}
...
MUTABLE_COLUMNS = {
    ...
    # P2b（0020）：connections 生命周期行（status CAS + version 递增，列级 UPDATE）
    "connections": {"status", "version", "updated_at"},
    # P2b（0021）：source_objects 仅 acl 可变（当前 ACL 权威，ADR-006 复检依赖）；
    # source_versions 仅生命周期列（内容列不可变——更新 = 新版本，0021 触发器）
    "source_objects": {"acl", "lifecycle_status", "last_sync_error", "updated_at"},
    "source_versions": {"state", "tombstone", "updated_at"},
    # P4（0023）：trigger state 仅值列与 updated_at 可变（key/scope 身份列不可变）
    "trigger_watermarks": {"value", "updated_at"},
    "memory_records": {
        # P4（0024，F-R6-05/D6）：canonical_value 加入可变集——仅限 active→
        # terminal 同一条 UPDATE 的 digest 占位覆写（0024 触发器 carve-out
        # 收窄：非占位形态/终态行/活跃行一律拒绝）。
        "approver_ref",
        "canonical_value",
        "confidence",
        "observed_at",

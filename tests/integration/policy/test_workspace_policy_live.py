...
def _memberships_app(stack: dict, actor: ActorContext) -> FastAPI:
    app = FastAPI()
    app.include_router(
...
class TestWorkspaceLifecycleUnderRealPolicy:
    ...
    async def test_org_membership_listing_read_authorization(self, stack) -> None:
        """org 级成员列表读路径授权（ADR-012 决策 4；D-1 反例的 org 级半边）。

        - 普通 member → 403：member 仅「读自身」，org 级枚举（含角色绑定分布）
          不在其矩阵 cell 内；
        - workspace_admin（workspace 作用域绑定）→ 403：org 级读取无 workspace
          上下文，作用域绑定不匹配（须走 workspace 级端点）；
        - org_owner / security_admin（org 作用域）→ 200。
        """
        org = stack["organization_id"]
        owner, _workspace_id = await self._owner_created_workspace(stack)
        ...
        async with _client(_memberships_app(stack, member_actor)) as client:
            response = await client.get(f"/api/v1/organizations/{org}/members")
        assert response.status_code == 403, response.text

        async with _client(_memberships_app(stack, other_ws_admin_actor)) as client:
            response = await client.get(f"/api/v1/organizations/{org}/members")
        assert response.status_code == 403, response.text

        async with _client(_memberships_app(stack, _org_owner(stack, owner))) as client:
            response = await client.get(f"/api/v1/organizations/{org}/members")
        assert response.status_code == 200, response.text

        async with _client(_memberships_app(stack, security_admin_actor)) as client:
            response = await client.get(f"/api/v1/organizations/{org}/members")
        assert response.status_code == 200, response.text

...
def test_source_delta_trigger_does_not_refire_on_unchanged_watermark() -> None:
    ...
    F-R5-05 RED 修订登记（P4 窗口 B4b，2026-09-08）：source_delta_changed
    由同步改为 async（watermark 持久化 store 为异步 PG 面——进程内 dict 即
    finding 所指缺口本体）；断言零变化（无 store 注入时进程内语义保持）。
    """
    manager, program, version, triggers = _active_program()
    program = manager.activate(program.id, performed_by="alice")
    ...
    async def flow() -> tuple[bool, bool, bool]:
        first = await service.source_delta_changed(program, version, triggers[2], observed="2025-12", now=_NOW)
        second = await service.source_delta_changed(program, version, triggers[2], observed="2025-12", now=_NOW)
        third = await service.source_delta_changed(program, version, triggers[2], observed="2026-01", now=_NOW)

...
class TestCacheCostTracking:
    def test_commit_records_cache_invalidated(self) -> None:
        """F-R5-03 RED 修订（P4 窗口 B3，2026-09-08）：原契约仅断言实例类型，
        放任 spec s3:116 必需的 cache_invalidated/预估重建成本在 manifest 构造
        处被静默丢弃（参数被接受、构造不含二者——「GREEN 恰好骗过测试」形态）。
        修订为字段值断言（passthrough True/False + 重建成本等式），isinstance
        断言保留、断言净加强（零删减）。"""
        compiler = _make_compiler()
        transition = EpochTransition(compiler)
        old_state = _make_state(items=(_make_item(),), sequence_no=0)
        ...
        assert isinstance(manifest, TransitionManifest)
        # spec s3 §4：两类跨 epoch 均记录 cache_invalidated 与预估重建成本
        assert manifest.cache_invalidated is True

        cost = {"weighted_tokens": 1200, "cache_read": 0}
        prep2 = transition.prepare(
            _make_state(items=(_make_item(),), sequence_no=1), direction="upgrade"
        )
        _, manifest2 = transition.commit(
            prep2, cache_invalidated=False, rebuild_cost_estimate=cost
        )
        assert manifest2.cache_invalidated is False
        assert manifest2.rebuild_cost_estimate == cost

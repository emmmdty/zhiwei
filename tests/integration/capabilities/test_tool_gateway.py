...
class TestSandboxSpec:
    ...
    def test_image_digest_format_enforced(self) -> None:
        for malformed in ("sha256:abc123", "not-a-digest", "sha256:" + "A" * 64):
            sandbox = SandboxSpec(
                image_digest=malformed,
                non_root=True,
                read_only_rootfs=True,
                no_docker_socket=True,
                no_network=True,
            ...
            violations = sandbox.validate_sandbox()
            assert any("digest" in v for v in violations), malformed
...
class TestDigestChain:
    """F-R2-02/F-R9-04：沙箱 digest 链——image_digest 必须取自 CapabilityVersion
    content_digest（构造期校验相等），随机伪造 digest 消失；未 pin/畸形 digest
    fail closed（SANDBOX_VIOLATION）。"""

    @pytest.fixture
    def pinned_capability(self, org_id: UUID, ws_id: UUID) -> CapabilityVersion:
        return CapabilityVersion(
            id=new_id(),
            capability_type="mcp",
            name="pinned-tool",
    ...
    def _recording_gateway(
        ...
        org_id: UUID,
        ws_id: UUID,
    ) -> tuple[ToolGateway, _RecordingPrebuiltRunner]:
        runner = _RecordingPrebuiltRunner(
            RunnerSpec(
                id=new_id(),
        ...
        gateway = ToolGateway(
            policy_enforcer=mock_policy_enforcer,
            runner_client=RunnerClient(registry, ipc_secret=b"test-secret"),
            invocation_repo=invocation_repo,
            connection_registry={connection.id: connection},
            credential_registry={credential.id: credential},
    ...
    async def _invoke(self, gateway: ToolGateway, capability: CapabilityVersion,
                      connection: Connection, credential: CredentialBinding,
                      org_id: UUID, ws_id: UUID, principal_id: UUID,
                      *, input_args: dict[str, object]) -> ToolInvocation:
        return await gateway.invoke(
            organization_id=org_id,
            workspace_id=ws_id,
            run_id=f"run-{input_args['query']}",
            task_id="task-1",
            attempt_no=1,
            tool_name="test-tool",
            ...
            policy_input=_make_policy_input(org_id, ws_id, capability.id, principal_id),
        )

    @pytest.mark.asyncio
    async def test_sandbox_digest_equals_capability_content_digest(
        self,
        pinned_capability: CapabilityVersion,
        connection: Connection,
        credential: CredentialBinding,
        mock_policy_enforcer: PolicyEnforcer,
        invocation_repo: InvocationRepository,
        ...
        gateway, runner = self._recording_gateway(
            pinned_capability, connection, credential, mock_policy_enforcer,
            invocation_repo, org_id, ws_id,
        )
        first = await self._invoke(
            gateway, pinned_capability, connection, credential, org_id, ws_id,
            principal_id, input_args={"query": "a"},
        )
        second = await self._invoke(
            gateway, pinned_capability, connection, credential, org_id, ws_id,
            principal_id, input_args={"query": "b"},
        )
        assert first.status == InvocationStatus.COMPLETED
        assert second.status == InvocationStatus.COMPLETED
        assert len(runner.received) == 2
        assert runner.received[0]["image_digest"] == _VALID_DIGEST
        assert runner.received[1]["image_digest"] == _VALID_DIGEST

    @pytest.mark.asyncio
    async def test_unpinned_content_digest_fails_closed(
        self,
        connection: Connection,
        credential: CredentialBinding,
        mock_policy_enforcer: PolicyEnforcer,
        ...
        gateway, runner = self._recording_gateway(
            capability, connection, credential, mock_policy_enforcer,
            invocation_repo, org_id, ws_id,
        )
        invocation = await self._invoke(
            gateway, capability, connection, credential, org_id, ws_id,
            principal_id, input_args={"query": "x"},
        )
        assert invocation.status == InvocationStatus.FAILED
        assert invocation.failure_reason == InvocationFailureReason.SANDBOX_VIOLATION
        assert runner.received == []

    @pytest.mark.asyncio
    async def test_malformed_content_digest_fails_closed(
        self,
        connection: Connection,
        credential: CredentialBinding,
        mock_policy_enforcer: PolicyEnforcer,
        ...
        invocation = await self._invoke(
            gateway, capability, connection, credential, org_id, ws_id,
            principal_id, input_args={"query": "y"},
        )
        assert invocation.status == InvocationStatus.FAILED
        assert invocation.failure_reason == InvocationFailureReason.SANDBOX_VIOLATION
        assert runner.received == []
...
class _RecordingPrebuiltRunner(PrebuiltRunner):
    """记录收到的 sandbox_spec 的 PrebuiltRunner 测试替身。"""
    ...
    async def _execute_in_sandbox(
        ...
        self.received.append(request.sandbox_spec)
        return await super()._execute_in_sandbox(request)
...
class TestBindingDigestRecheck:
    """F-R9-03/T-P3.4：invocation 前绑定 digest 复核——capability 钉住的 digest 与
    provider 当前内容 digest 失配（准入后被篡改/上游换内容）即拒绝执行。"""
    ...
    def _gateway(
        ...
        registry = RunnerRegistry()
        registry.register(PrebuiltRunner(self._runner_spec()))
        return ToolGateway(
            policy_enforcer=mock_policy_enforcer,
            runner_client=RunnerClient(registry, ipc_secret=b"test-secret"),
            invocation_repo=invocation_repo,
    ...
    def _runner_spec(self) -> RunnerSpec:
        return RunnerSpec(
            id=new_id(),
            name="test-runner",
            kind=RunnerKind.PREBUILT,
    ...
    def _provider(self, content: dict[str, object]) -> ProviderVersion:
    ...
    @pytest.mark.asyncio
    async def test_drifted_provider_content_refuses_invocation(
        self,
        connection: Connection,
        credential: CredentialBinding,
        mock_policy_enforcer: PolicyEnforcer,
        ...
        drifted = self._provider(
            {"tools": [{"name": "echo", "description": "tampered"}]}
        )
        assert drifted.content_digest != _VALID_DIGEST
        gateway = self._gateway(
            capability, drifted, connection, credential,
            mock_policy_enforcer, invocation_repo,
        )
        invocation = await self._invoke(
            gateway, capability, drifted, connection, credential,
            org_id, ws_id, principal_id,
        )
        assert invocation.status == InvocationStatus.FAILED
        assert (
            invocation.failure_reason == InvocationFailureReason.CAPABILITY_DRIFTED
        )

    @pytest.mark.asyncio
    async def test_matching_provider_content_passes(
        self,
        connection: Connection,
        credential: CredentialBinding,
        mock_policy_enforcer: PolicyEnforcer,
        ...
        provider = self._provider({"tools": [{"name": "echo"}]})
        capability = capability.model_copy(
            update={"content_digest": provider.content_digest}
        )
        gateway = self._gateway(
            capability, provider, connection, credential,
            mock_policy_enforcer, invocation_repo,
        )
        invocation = await self._invoke(
            gateway, capability, provider, connection, credential,
            org_id, ws_id, principal_id,
        )
        assert invocation.status == InvocationStatus.COMPLETED

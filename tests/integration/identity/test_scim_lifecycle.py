...
class FakeOPA:
    ...
    def __init__(self) -> None:
        # F-R3-05 残留收口：缺省 deny，allow 须显式声明（fail closed 测试纪律）
...
async def _seed_membership(principal_id: UUID, organization_id: UUID, roles: list[str]) -> None:
    """直接插 membership 行（org 级）。

    ON CONFLICT DO NOTHING：对同一 (principal, org) 已有 SCIM provision-time
    空绑定行时，本 helper 的角色 seed 会被吞（保持 []）——需要角色的测试须
    在 SCIM provisioning 之前 seed，或改用 member-add API。
    """
    connection = await asyncpg.connect(ADMIN_DSN)
    try:
        await connection.execute(
...
async def test_scim_user_create_writes_principal_identity_and_audit(
    ...
    opa.allow = True  # 显式声明 allow（缺省 deny——F-R3-05）
...
async def test_scim_reads_and_mutations_are_policy_gated(
    ...
    _app, client, idp, opa = app_and_client  # 缺省 deny（policy-gated 契约）
...
async def test_scim_opa_unavailable_fails_closed(
    ...
    _app, client, idp, opa = app_and_client  # fail-closed 契约：保持缺省 deny
    owner_id = await _reset_alice()
    org = await _seed_org()
    await _seed_membership(owner_id, org, ["org_owner"])
    await _perform_login(client, idp)
...
@pytest.mark.asyncio
async def test_scim_user_cross_org_idor_404(
    migrated_database: None,
    app_and_client: tuple[FastAPI, httpx.AsyncClient, FakeIdP, FakeOPA],
) -> None:
    """F-R3-08 RED：User 读按 org membership 关联过滤（ADR-012 决策 4）。

    跨 org principal 与不存在同文案 404（防枚举）；同 org membership 持有者
    正常可读（正向控制）。PATCH 同样受过滤——principal 是 identity-global
    资源，跨 org 状态迁移（disable/enable）与读同一缺口。
    """
    app, client, idp, opa = app_and_client
    opa.allow = True  # 显式声明 allow（缺省 deny——F-R3-05）
    owner_a = await _reset_alice()
    org_a = await _seed_org()
    await _seed_membership(owner_a, org_a, ["org_owner"])
    victim = await _seed_principal("victim-oidc")
    await _seed_membership(victim, org_a, ["member"])
    await _perform_login(client, idp)
    csrf_a = await _csrf_token(client)

    # 正向控制：membership 关联 org 的 owner 可读
    ok = await client.get(
        f"/scim/v2/Users/{victim}",
        headers=_scim_headers(csrf_a, organization_id=org_a),
    )
    ...
    async for client_b in _client(app):
        ...
        _assert_scim_error(
            await client_b.get(
                f"/scim/v2/Users/{victim}",
                headers=_scim_headers(csrf_b, organization_id=org_b),
            ),
            404,
        )
        _assert_scim_error(
...
@pytest.mark.asyncio
async def test_scim_provisioned_user_scoped_to_provisioning_org(
    migrated_database: None,
    app_and_client: tuple[FastAPI, httpx.AsyncClient, FakeIdP, FakeOPA],
) -> None:
    """F-R3-08 RED：provisioned user 的可读面锚定供给 org。

    SCIM provisioning（POST /Users）是 org 作用域动作：供给的 principal 与供给
    org 建立 Membership 关联（role_bindings 为空 = 无角色、仅组织归属），因此
    供给 org 的 get-after-create 旅程保持 200，其他 org 一律 404。
    """
    app, client, idp, opa = app_and_client
    opa.allow = True  # 显式声明 allow（缺省 deny——F-R3-05）
    owner_a = await _reset_alice()
    ...
    created = await _create_user(client, csrf_a, org_a, "provisioned")
    user_id = UUID(created["id"])
...
@pytest.mark.asyncio
async def test_scim_provision_disabled_user_gains_affiliation(
    migrated_database: None,
    app_and_client: tuple[FastAPI, httpx.AsyncClient, FakeIdP, FakeOPA],
) -> None:
    """对抗审查 gap 1 回归锚点：active=false 供给（建后禁用）必须取得归属。

    disable-at-creation 是合法 IdP 供给流；principal 由本请求创建（liveness
    自证），membership 在 disable 后存活是域语义——归属缺失会让该 principal
    对供给 org 永久不可见且无法经 SCIM enable（404 挡住 PATCH）。
    """
    app, client, idp, opa = app_and_client
    opa.allow = True  # 显式声明 allow（缺省 deny——F-R3-05）
    owner_a = await _reset_alice()
    ...
    user_id = UUID(created_response.json()["id"])

    # 供给 org 可见（active=false 如实呈现）
    ok = await client.get(
        f"/scim/v2/Users/{user_id}",
        headers=_scim_headers(csrf_a, organization_id=org_a),
    )
    ...
    # 供给 org 可 enable（PATCH 不被 404 挡住）
    reenable = await client.patch(
        f"/scim/v2/Users/{user_id}",
        headers=_scim_headers(csrf_a, organization_id=org_a),
        json=_patch_active(True),
    )
...
class TestProvisioningMembershipAnchors:
    """F-R3-08 锚点债（A2a 验收登记，P2b 补齐）：provision-time membership
    语义的三项直接锚点——① 空绑定断言；② provisioned user 出现在 org members
    列表；③ provisioned 身份登录后的 org 上下文收敛（resolve_context 对
    供给 org 成功 = membership 关联生效；空绑定 → 策略面 403 而非租户面 404）。
    """

    @pytest.mark.asyncio
    async def test_provisioned_user_membership_anchors(
        self,
        migrated_database: None,
        app_and_client: tuple[FastAPI, httpx.AsyncClient, FakeIdP, FakeOPA],
    ) -> None:
        ...
        created = await client.post(
            "/scim/v2/Users",
            headers=_scim_headers(csrf, organization_id=org),
            json=_user_body("anchor-u1"),
        )
        assert created.status_code == 201, created.text
        user_id = UUID(created.json()["id"])

        # ① 空绑定：membership 存在且 role_bindings 为空
        import asyncpg as _asyncpg

        _conn = await _asyncpg.connect(ADMIN_DSN)
        try:
            bindings = await _conn.fetchval(
                "SELECT role_bindings FROM memberships"
        ...
        assert bindings == [] or bindings == "[]"

        # ② org members 列表包含 provisioned user
        listing = await client.get(
            f"/api/v1/organizations/{org}/members",
            headers=_scim_headers(csrf, organization_id=org),
        )
        assert listing.status_code == 200, listing.text
        member_ids = {m["principal_id"] for m in listing.json()}
        assert str(user_id) in member_ids

        # ③ provisioned 身份登录后 org 上下文收敛：resolve_context 对供给
        # org 成功（membership 关联生效）。判别器：切回缺省 deny——上下文
        # 收敛时 PEP 求值到达 fake → 403 "policy denied"（空绑定无角色）；
        # 未收敛则 MembershipScopeError → 404 "resource not found"。
        # callback 总是签发全新 session（防 fixation）——直接重新登录即可
        opa.allow = False
        await _perform_login(client, idp, subject="anchor-u1")
        provisioned_csrf = await _csrf_token(client)
        resp = await client.get(
            f"/scim/v2/Users/{user_id}",
            headers=_scim_headers(provisioned_csrf, organization_id=org),
        )

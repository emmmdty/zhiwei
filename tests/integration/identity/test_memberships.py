...
async def test_api_organization_bootstrap_and_list(
    ...
    app = FastAPI()
    app.include_router(
        create_organizations_router(actor_dependency=lambda: actor, sessions=sessions, policy_enforcer=FakePolicyEnforcer(allow=True))
    )
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
...
async def test_api_organization_read_is_scoped_to_actor_org(
    ...
    no_org_app.include_router(
        create_organizations_router(actor_dependency=_no_org_actor, sessions=sessions, policy_enforcer=FakePolicyEnforcer(allow=True))
    )
    async with AsyncClient(
...
async def test_api_members_endpoints_enforce_scope(
    ...
    no_org_app = FastAPI()
    no_org_app.include_router(
        create_memberships_router(actor_dependency=_no_org_actor, sessions=sessions, policy_enforcer=FakePolicyEnforcer(allow=True))
    )
    async with AsyncClient(
        transport=ASGITransport(app=no_org_app), base_url="http://test"
    ) as client:
...
@pytest.mark.asyncio
async def test_org_member_endpoints_work_with_workspace_declared_context(
    migrated_database: None,
    sessions: async_sessionmaker[AsyncSession],
    identity_sessions: async_sessionmaker[AsyncSession],
) -> None:
    """S11 followup-2 任务三实测缺陷（产品栈 capture journey 暴露）。

    前端选中 workspace 后全局携带 X-ZhiWei-Workspace（session meta 注入），
    /organizations/{org}/members 的 GET/POST/DELETE 是 org 级资源读写——PEP
    gate 本就按 workspace_id=None 求值，RLS 上下文必须与被读资源对齐
    （ADR-014 同款原则），不得沿用 actor 的 ws 声明；否则 repository 的
    _require_org_level 守卫把 org 读全拒成 404，前端角色解析在 ws 选中后
    降级为空（Signed in as 角色消失、owner 入口消失）。
    """
    organization_id = uuid4()
    workspace_id = uuid4()
    await _seed_organization_and_workspace(
    ...
    member_id = uuid4()
    await _seed_principal(identity_sessions, member_id)
    # 真实 UI 形态：org owner 兼持 demo workspace 的 ws 绑定（/me 带头回显后）
    actor = _org_actor(organization_id, workspace_id=workspace_id)
    app = FastAPI()
    app.include_router(
        create_memberships_router(
    ...
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        listed = await client.get(f"/api/v1/organizations/{organization_id}/members")
        assert listed.status_code == 200, listed.text

        added = await client.post(
            f"/api/v1/organizations/{organization_id}/members",
            json={"principal_id": str(member_id), "role_bindings": ["member"]},
            headers={"Idempotency-Key": "ws-context-add-1"},
        )
        ...
        assert [m["principal_id"] for m in listed_after.json()] == [str(member_id)]

        removed = await client.delete(
            f"/api/v1/organizations/{organization_id}/members/{member_id}",
            headers={"Idempotency-Key": "ws-context-remove-1"},
        )

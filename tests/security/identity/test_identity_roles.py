...
async def test_membership_resolver_function_structure(migrated_database: None) -> None:
    ...
    # 修订（0027_rls_resolver，产品化窗口 2026-09-08）：owner 由 zhiwei_migrator
    # 转移到 zhiwei_rls_resolver（NOLOGIN BYPASSRLS）——SECURITY DEFINER 以 owner
    # 执行，FORCE RLS 对非超级用户 definer 生效，原 owner 在产品姿态下恒返回空；
    # 旁路面收窄到该函数体（PG 禁止 definer 内 SET role，所有权转移是唯一机制）。
    # 契约意图不变：definer 特权 + 窄接口，invoker 特权不变。
    assert meta["owner"] == "zhiwei_rls_resolver"
    assert meta["prosecdef"] is True
    config = meta["proconfig"]
    assert config is not None, "必须固定 search_path"
    assert any("search_path" in entry for entry in config)
    ...
    # 修订（0027_rls_resolver）：zhiwei_app 不再被排除——分库姿态（业务/identity
    # 分离）下 membership 数据在业务库，resolver 由业务数据面角色 zhiwei_app 调用
    # （invoker 仅获得 EXECUTE，无任何表权限；旁路只随 definer 的窄角色生效）。
    assert "zhiwei_app" in acl
...
async def test_execute_grants_are_per_role(migrated_database: None) -> None:
    ...
    try:
        ...
        # 修订（0027_rls_resolver）：zhiwei_app 由「不得调用」改为合法调用方——
        # 分库姿态下 membership 数据在业务库，resolver 经业务引擎由 zhiwei_app
        # 调用（invoker 仅 EXECUTE，无表权限；跨租户旁路只随 definer 窄角色）。
        assert await connection.fetchval(
...
async def test_app_role_cannot_call_membership_resolver(migrated_database: None) -> None:
    """修订（0027_rls_resolver）：原断言「app 角色不得调用 resolver」反映的是
    单库姿态假设；分库姿态下 membership 数据在业务库、resolver 由业务数据面角色
    zhiwei_app 调用（见 sessions.memberships）。边界翻转为正向：app 可调用并取得
    绑定行；无绑定 principal 返回空而非泄露他租户行。PUBLIC 仍无 EXECUTE
    （test_execute_grants_are_per_role）。"""
    principal_id = uuid4()
    await _seed_identity_rows(principal_id)
    connection = await asyncpg.connect(APP_DSN)
    try:
        ...
        assert len(scopes) >= 1, f"app 调用 resolver 必须返回绑定摘要: {rows}"
    finally:
        await connection.close()

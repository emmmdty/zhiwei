...
async def test_api_cross_org_workspace_write_403_and_list_404(
    ...
    await _seed_two_org_world()
    app = FastAPI()
    app.include_router(create_workspaces_router(actor_dependency=_alice_org_actor, sessions=sessions, policy_enforcer=FakePolicyEnforcer(allow=True)))
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_api_workspace_level_cross_org_groups_404_403_and_no_name_leak(
    ...
    app = FastAPI()
    app.include_router(
        create_workspaces_router(actor_dependency=_alice_workspace_actor, sessions=sessions, policy_enforcer=FakePolicyEnforcer(allow=True))
    )
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_membership_and_workspace_membership_idor_blocked(
    ...
    app = FastAPI()
    app.include_router(create_memberships_router(actor_dependency=_alice_org_actor, sessions=sessions, policy_enforcer=FakePolicyEnforcer(allow=True)))
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:

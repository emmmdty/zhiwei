...
# 基线跟随最近一次已提交的迁移头（S11-T3：0026 contract）。本文件的契约是
# release readiness 语义，不依赖具体 revision——pin 的目的是隔离并发 Task 的
# WIP 迁移（pin 之后的头是不可控输入）；DB 已在基线之后时绝不 downgrade
# （对共享测试库非破坏）。链内序号同前缀单调，字典序比较在本迁移链上等价
# 于拓扑序。S11-T3 修订：0016→0026（outbox.dispatch_deadline 三段式落库，
# ORM 模型列与已提交头保持一致——tests/integration/foundation 的
# metadata-match 契约要求）。
_BASELINE_REVISION = "0026_contract_dispatch_deadline"

...
def _runs_app(stack: dict, actor: ActorContext, *, queue: str | None = None) -> FastAPI:
    ...
    app.include_router(
        create_runs_router(
            ...
            # PEP 接线（F-R3-02）：fake 恒 allow，矩阵语义由 Rego 套件钉死；
            # deny 面行为由 TestPolicyWiring 的 allow=False 用例覆盖
...
class TestPolicyWiring:
    """P2a.1 RED（F-R3-02 / F-R2-13）：runs 面接 PEP + 决策端点主体断言与审计。

    finding 证据（2026-09-06 评审）：GET /runs、GET /runs/{id}、GET /runs/{id}/events、
    GET /runs/{id}/approvals 全部无 PEP；POST decision 仅 _tenant + store SoD
    （approvals.py:142-145），无 OPA、无审计、无 principal-kind 断言。契约：
    specs/s1-tenancy-policy.md:47 与 ADR-012 决策 4——runs/approvals 读走 PEP read
    cell（run_case_artifact.read），不允许仅依赖 RLS+membership 判定可见性；
    审批决策接 tool_approval.approve/reject（当事人证据经 F-R3-06 的
    resource_context 通道从 approval 权威记录解析）+ USER principal 断言 + 同事务
    allowed / 独立事务 denied・failed 审计。

    RED 失败模式：工厂不接受 policy_enforcer（TypeError unexpected keyword）=
    接线缺失本体；kind 断言与审计用例失败于现行为（404 / 零审计行）。
    """

    AUDIT_ACTION = "run.approval.decide"

    def _enforcer_app(self, stack: dict, actor: ActorContext, enforcer: Any) -> FastAPI:
        sessions = stack["sessions"]

        def sessions_factory(actor: ActorContext, workspace_id: UUID | None):
            return sessions
    ...
    async def _audit_rows(self, stack: dict, action: str) -> list[AuditEvent]:
        context = stack["context"]
        async with tenant_session(stack["sessions"], context) as session:
            rows = (
    ...
    async def _create_run_with_pending_approval(
        self, stack: dict, client: AsyncClient, creator: UUID
    ) -> tuple[str, str]:
        context = stack["context"]
        created = await client.post(
            "/api/v1/runs",
    ...
    async def test_factory_requires_policy_enforcer(self, stack) -> None:
        sessions = stack["sessions"]
        ...
        with pytest.raises(TypeError):
            create_runs_router(**common)
        with pytest.raises(TypeError):
            create_runs_router(policy_enforcer=None, **common)  # type: ignore[arg-type]

    @pytest.mark.parametrize(
        "path",
        [
            lambda: "/api/v1/runs",
            lambda: f"/api/v1/runs/{uuid4()}",
            lambda: f"/api/v1/runs/{uuid4()}/events",
            lambda: f"/api/v1/runs/{uuid4()}/approvals",
        ],
        ids=["list", "get_run", "events", "approvals"],
    )
    async def test_reads_denied_without_read_cell(self, stack, path: Any) -> None:
        creator = uuid4()
        await _grant_workspace_membership(
            stack["identity_sessions"], stack["sessions"], stack["context"], creator
        )
        app = self._enforcer_app(
            stack, _actor(stack["context"], creator), FakePolicyEnforcer(allow=False)
        )
    ...
    async def test_reads_allowed_with_enforcer(self, stack) -> None:
        creator = uuid4()
        await _grant_workspace_membership(
            stack["identity_sessions"], stack["sessions"], stack["context"], creator
        )
        app = self._enforcer_app(
            stack, _actor(stack["context"], creator), FakePolicyEnforcer(allow=True)
        )
        async with AsyncClient(
            transport=ASGITransport(app=app), base_url="http://test"
        ) as client:
    ...
    async def test_read_policy_input_shape(self, stack) -> None:
        """A2a 残留②（读路径 PolicyInput 形状锚点）：runs 列表读的
        PolicyInput resource 形状——type=run_case_artifact、action=read、
        resource_id=actor org（列表无单资源 id 的约定见 runs.py:_authorize_run_read）。"""
        ...
        def handler(request: _httpx.Request) -> _httpx.Response:
            inputs.append(_json.loads(request.read())["input"])
            return _httpx.Response(
                200,
                json={
                    "decision_id": "decision-allow-1",
                    "result": {"allow": True, "reason": "allow:matrix"},
                    "provenance": {
                        "version": "1.19.0",
        ...
        app = self._enforcer_app(
            stack, _actor(stack["context"], creator), PolicyEnforcer(opa_client)
        )
        async with AsyncClient(
            transport=ASGITransport(app=app), base_url="http://test"
        ) as client:
        ...
        assert policy_input["action"] == "read"
        assert policy_input["resource"]["type"] == "run_case_artifact"
        assert policy_input["resource"]["id"] == str(stack["context"].organization_id)
        await opa_client.aclose()

    async def test_decision_denied_without_approve_cell(self, stack) -> None:
        creator, approver = uuid4(), uuid4()
        for principal in (creator, approver):
            await _grant_workspace_membership(
                stack["identity_sessions"], stack["sessions"], stack["context"], principal
            )
        context = stack["context"]
        async with stack["worker"], AsyncClient(
            transport=ASGITransport(
                app=self._enforcer_app(
                    stack, _actor(context, creator), FakePolicyEnforcer(allow=True)
                )
            ),
            base_url="http://test",
        ) as creating:
            run_id, request_id = await self._create_run_with_pending_approval(
                stack, creating, creator
            )
        async with AsyncClient(
            transport=ASGITransport(
                app=self._enforcer_app(
                    stack, _actor(context, approver), FakePolicyEnforcer(allow=False)
                )
            ),
            base_url="http://test",
        ) as deciding:
            response = await deciding.post(
                f"/api/v1/runs/{run_id}/approvals/{request_id}/decision",
                json={"decision": "approved", "reason": "denied by policy"},
            )
        assert response.status_code == 403, response.text
        assert response.json() == {"detail": "policy denied"}
        # 被拒决策必须留 denied 审计（独立事务），且审批保持 pending
        rows = await self._audit_rows(stack, self.AUDIT_ACTION)
        assert rows and rows[-1].result == "denied", rows
        async with AsyncClient(
            transport=ASGITransport(
                app=self._enforcer_app(
                    stack, _actor(context, approver), FakePolicyEnforcer(allow=True)
                )
            ),
            base_url="http://test",
        ) as verifying:
            listing = await verifying.get(f"/api/v1/runs/{run_id}/approvals")
        assert listing.json()[0]["status"] == "pending", listing.text

    async def test_decision_threads_record_evidence_and_writes_allowed_audit(
        self, stack
    ) -> None:
        creator, approver = uuid4(), uuid4()
        for principal in (creator, approver):
        ...
        async with stack["worker"], AsyncClient(
            transport=ASGITransport(
                app=self._enforcer_app(stack, _actor(context, creator), enforcer)
            ),
            base_url="http://test",
        ) as creating:
            run_id, request_id = await self._create_run_with_pending_approval(
        ...
        async with AsyncClient(
            transport=ASGITransport(
                app=self._enforcer_app(stack, _actor(context, approver), enforcer)
            ),
            base_url="http://test",
        ) as deciding:
            decision = await deciding.post(
        ...
        assert decision.status_code == 200, decision.text
        # PEP 收到的 PolicyInput 必须携带 tool_approval cell + 权威记录当事人证据
        observed = enforcer.inputs[-1]
        assert observed.resource.type == "tool_approval", observed
        assert observed.action == "approve", observed
        assert observed.resource_context.requester_principal_id == creator, observed
        assert observed.resource_context.modifier_principal_ids == (creator,), observed
        # 决策留 allowed 审计（同事务），decision metadata 来自真实决策
        rows = await self._audit_rows(stack, self.AUDIT_ACTION)
        allowed = [row for row in rows if row.result == "allowed"]
        assert allowed, rows
        row = allowed[-1]
        assert row.resource_type == "approval_request", row
        assert str(row.resource_id) == request_id, row
        assert row.decision_id is not None and row.policy_revision is not None, row
        assert row.request_id is not None and row.trace_id is not None, row

    async def test_decision_by_service_account_is_403(self, stack) -> None:
        """F-R2-13：非 USER principal 不得决策（未来 service-account 认证路径的
        SoD「human」假设报警）。拒绝必须写 denied 审计。"""
        actor = ActorContext(
            principal_id=uuid4(),
            organization_id=stack["context"].organization_id,
            workspace_id=stack["context"].workspace_id,
            kind=PrincipalKind.SERVICE_ACCOUNT,
        )
        app = self._enforcer_app(stack, actor, FakePolicyEnforcer(allow=True))
        async with AsyncClient(
            transport=ASGITransport(app=app), base_url="http://test"
        ) as client:
            response = await client.post(
                f"/api/v1/runs/{uuid4()}/approvals/{uuid4()}/decision",
                json={"decision": "approved", "reason": "machine decide"},
            )
        assert response.status_code == 403, response.text
        rows = await self._audit_rows(stack, self.AUDIT_ACTION)
        assert rows and rows[-1].result == "denied", rows

    async def test_sod_conflict_writes_failed_audit(self, stack) -> None:
        """PEP 放行后 store 层 SoD 拒绝（409）→ failed 审计（独立事务）。"""
        creator = uuid4()
        await _grant_workspace_membership(
            stack["identity_sessions"], stack["sessions"], stack["context"], creator
        )
        context = stack["context"]
        async with stack["worker"], AsyncClient(
            transport=ASGITransport(app=_runs_app(stack, _actor(context, creator))),
            base_url="http://test",
        ) as client:
            run_id, request_id = await self._create_run_with_pending_approval(
                stack, client, creator
            )
            self_decision = await client.post(
                f"/api/v1/runs/{run_id}/approvals/{request_id}/decision",
                json={"decision": "approved", "reason": "self approve"},
        ...
        assert failed, rows
        assert str(failed[-1].resource_id) == request_id, failed[-1]


class TestRunStartIdempotency:
    """F-R5-05（T-P4.4/B4a）：POST /runs 接 Idempotency-Key（scope=run.start）
    ——客户端超时重试不得产生重复 Run；同键异请求体 409；无键行为不变。"""

    async def _post(self, client, context, *, key, template="single-fixture"):
        headers = {"Idempotency-Key": key} if key is not None else {}
        return await client.post(
            "/api/v1/runs",
            json={"template": template, "workspace_id": str(context.workspace_id)},
            headers=headers,
        )

    async def _run_count(self, stack) -> int:
        from sqlalchemy import func

        from zhiwei.persistence.models import Run

        async with tenant_session(stack["sessions"], stack["context"]) as session:
            return (await session.scalar(select(func.count()).select_from(Run))) or 0

    async def test_retry_with_same_key_returns_original_run_no_duplicate(
        self, stack
    ) -> None:
        creator = uuid4()
        await _grant_workspace_membership(
        ...
        async with stack["worker"], AsyncClient(
            ...
            first = await self._post(client, context, key="run-start-key-1")
            assert first.status_code == 201, first.text
            second = await self._post(client, context, key="run-start-key-1")
            assert second.status_code == 200, second.text
            assert second.json()["run_id"] == first.json()["run_id"]
            assert second.json()["status"] == "created"
        assert await self._run_count(stack) == 1

    async def test_same_key_different_body_is_409(self, stack) -> None:
        creator = uuid4()
        await _grant_workspace_membership(
            stack["identity_sessions"], stack["sessions"], stack["context"], creator
        )
        ...
        async with stack["worker"], AsyncClient(
            ...
            first = await self._post(client, context, key="run-start-key-2")
            assert first.status_code == 201, first.text
            conflict = await self._post(
                client, context, key="run-start-key-2", template="approval-chain"
            )
            assert conflict.status_code == 409, conflict.text

    async def test_missing_key_keeps_legacy_behavior(self, stack) -> None:
        creator = uuid4()
        await _grant_workspace_membership(
            stack["identity_sessions"], stack["sessions"], stack["context"], creator
        )
        ...
        async with stack["worker"], AsyncClient(
            ...
            first = await self._post(client, context, key=None)
            second = await self._post(client, context, key=None)
            assert first.status_code == 201 and second.status_code == 201
            assert first.json()["run_id"] != second.json()["run_id"]
        assert await self._run_count(stack) == 2

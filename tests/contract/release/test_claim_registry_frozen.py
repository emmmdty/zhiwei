...
class TestLiveDirectEdge:
    """S11 live 演示任务的 ADR-016 修订：live-production 作用域 claim 的直达边。

    反例（状态机缺口）：live-production 口径的 claim 走
    planned→implemented→offline_verified 时，mix-rule（live-production 拒绝
    offline 证据）使 offline 步不可构造——live claim 在原状态机下不可产生。
    修订：新增 (IMPLEMENTED, LIVE_VERIFIED) 证据边（仅 live/shadow），mix-rule
    在边处理内保持原语义（offline 证据仍被拒）。
    """

    def test_implemented_live_claim_upgrades_with_live_seal(self) -> None:
        upgraded = upgrade_claim(
            _claim(status=ClaimStatus.IMPLEMENTED, environment="live-production"),
            _evidence(mode="live"),
            target=ClaimStatus.LIVE_VERIFIED,
        )
        assert upgraded.status == ClaimStatus.LIVE_VERIFIED

    def test_implemented_live_claim_with_shadow_seal(self) -> None:
        upgraded = upgrade_claim(
            _claim(status=ClaimStatus.IMPLEMENTED, environment="live-production"),
            _evidence(mode="shadow"),
            target=ClaimStatus.LIVE_VERIFIED,
    ...
    def test_implemented_offline_claim_still_refuses_jump_to_live(self) -> None:
        # 非 live-production 作用域的 implemented claim 仍不得跳级（边只对
        # 证据模式开放，不放宽作用域纪律）
        with pytest.raises(ClaimUpgradeDenied):
            upgrade_claim(
                _claim(status=ClaimStatus.IMPLEMENTED, environment="offline-fixture"),
                _evidence(mode="live"),
                target=ClaimStatus.LIVE_VERIFIED,
            )

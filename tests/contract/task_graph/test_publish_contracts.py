...
class TestPublishBoundaryValidation:
    ...
    def test_heterogeneous_strategies_for_shared_field_rejected(self) -> None:
        """B2 审查响应（F-R1-03 MAJOR 根因①）：并行写者对同一字段声明
        不同策略——增补 2「K 个节点都必须声明**与字段一致的** merge 策略」
        字面要求同质；异构声明使运行时合并语义无定义（APPEND×LWW 混合）。"""
        graph = TaskGraph(
            nodes={
                "t_a": _node("t_a", parallel=True, merge={"f": MergeStrategy.APPEND},
                             outputs={"f": "x"}),
                "t_b": _node("t_b", parallel=True, merge={"f": MergeStrategy.LAST_WRITE_WINS},
                             outputs={"f": "y"}),
            },
            edges={},
        )
        with pytest.raises(ValueError, match="consistent"):
            graph.validate_merge_strategies()

...
经本 fake 接线。fake 不实现任何授权语义：allow 显式声明（allow=True）时
authorize 恒 allow（携带非空 decision_id/revision，满足 v2 审计 metadata 契约），
其余一切决策为 fail-closed deny（metadata 全 NULL）。

默认 deny（F-R3-05）：与生产 PEP 的 fail-closed 纪律同构——新 router 的集成
测试若只配 fake 即默认放行，矩阵 deny 语义不会在 wiring 层被意外发现；allow
必须显式声明，且新 router 集成测试必须同时含 allow=False 用例
（tests/integration/telemetry/test_observability_api.py 为范例）。
...
class FakePolicyEnforcer(PolicyEnforcer):
    """allow 显式声明（allow=True）才恒 allow 的测试 fake；默认 deny（F-R3-05）。"""

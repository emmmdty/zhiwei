...
class TestTerminalRedaction:
    """F-R6-05（D6 决议）：终态转移覆写内容列为 digest 占位（redaction +
    boundary）——「删除」的字面承诺与实现一致，受控个人信息不在存储驻留。

    范围 = 遗忘语义（REVOKED/EXPIRED）；SUPERSEDED 保留原文（纠正语义，
    判分器 _unit_supersede（evals/executors/memory.py:513 superseded_value）
    读取 superseded 内容——P5 spec 修订显式口径）。占位形态 = "redacted:sha256:<64hex>"，digest 为原内容
    SHA-256（可验证性 boundary，PERMISSIONS「保留必要 digest/redacted
    marker」）。"""

    PLACEHOLDER_PATTERN = r"^redacted:sha256:[0-9a-f]{64}$"

    def test_revoke_redacts_content_with_verifiable_digest(self) -> None:
        from zhiwei.contracts.canonical import digest

        queue = CandidateQueue()
        record = _make_record()
        record = record.model_copy(update={"canonical_value": "dark secret"})
        queue.add_candidate(record)
        dedup = DedupKey.from_record(record)

        revoked = queue.revoke_record(dedup, "user requested deletion")
        assert revoked is not None
        assert revoked.canonical_value == f"redacted:{digest('dark secret')}"
        assert revoked.tombstone is True
        assert revoked.status == MemoryStatus.REVOKED

    def test_expire_redacts_content(self) -> None:
        from zhiwei.contracts.canonical import digest

        queue = CandidateQueue()
        record = _make_record()
        record = record.model_copy(update={"canonical_value": "stale secret"})
        queue.add_candidate(record)

        expired = queue.expire_candidates(
            record.created_at + RetentionPolicy().candidate_ttl + timedelta(seconds=1)
        )
        assert len(expired) == 1
        assert expired[0].canonical_value == f"redacted:{digest('stale secret')}"
        assert expired[0].tombstone is True

    def test_confirm_keeps_content(self) -> None:
        queue = CandidateQueue()
        record = _make_record(status=MemoryStatus.CANDIDATE)
        queue.add_candidate(record)
        confirmed = queue.confirm_candidate(
            DedupKey.from_record(record), approver_id=new_id()
        )
        assert confirmed is not None
        assert confirmed.canonical_value == "test_value"

    def test_supersede_keeps_original_content(self) -> None:
        queue = CandidateQueue()
        original = _make_record()
        queue.add_candidate(original)
        correction = _make_record(key="test.key").model_copy(
            update={"canonical_value": "corrected"}
        )
        superseded, _confirmed = queue.supersede_record(
            DedupKey.from_record(original), correction
        )
        assert superseded.canonical_value == "test_value"
        assert superseded.status == MemoryStatus.SUPERSEDED

    def test_placeholder_shape_is_digest_only(self) -> None:
        """占位不携带原文碎片：仅 redacted 前缀 + 内容 digest。"""
        import re
        ...
        queue = CandidateQueue()
        record = _make_record()
        record = record.model_copy(update={"canonical_value": "pii: 12-34-5678"})
        queue.add_candidate(record)
        revoked = queue.revoke_record(DedupKey.from_record(record), "gdpr")
        assert revoked is not None
        assert re.fullmatch(self.PLACEHOLDER_PATTERN, revoked.canonical_value)
        assert "12-34-5678" not in revoked.canonical_value
        assert revoked.canonical_value == f"redacted:{digest('pii: 12-34-5678')}"

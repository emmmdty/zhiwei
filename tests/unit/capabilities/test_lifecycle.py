...
class TestCapabilityVersionManager:
    ...
    def test_check_transition_valid_is_silent(self) -> None:
        # F-R9-01 前置检查：结构合法性在审批评估之前判定（API 层 409 先于 403）
        mgr = CapabilityVersionManager()
        v = mgr.register("mcp", "tool")
        mgr.check_transition(v.id, CapabilityStatus.QUARANTINED)

    def test_check_transition_skip_step_raises_without_mutation(self) -> None:
        mgr = CapabilityVersionManager()
        v = mgr.register("mcp", "tool")
        with pytest.raises(InvalidTransitionError):
            mgr.check_transition(v.id, CapabilityStatus.APPROVED)
        assert mgr._get_version(v.id).status == CapabilityStatus.DISCOVERED

    def test_check_transition_unknown_version_raises(self) -> None:
        mgr = CapabilityVersionManager()
        with pytest.raises(NotFoundError):
            mgr.check_transition(uuid4(), CapabilityStatus.PUBLISHED)
    ...
    def test_cas_publish_success(self) -> None:
        # RED 修订（F-R9-06，2026-09-06）：原契约 expected_version=v.version 与
        # 业务版本号（生命周期内恒 1）比较，正是 finding 指出的无效 CAS——比较
        # 对象从不变化，永远无法检测并发。修订为与 row_version（随每次 transition
        # 递增的乐观并发基准）比较。
        mgr = CapabilityVersionManager()
        v = mgr.register("mcp", "tool")
        v = mgr.transition(v.id, CapabilityStatus.QUARANTINED)
        ...
        v = mgr.transition(v.id, CapabilityStatus.APPROVED)
        v = mgr.transition(
            v.id, CapabilityStatus.PUBLISHED, expected_version=mgr.row_version(v.id)
        )
        assert v.status == CapabilityStatus.PUBLISHED

    def test_cas_publish_detects_stale_row_version(self) -> None:
        """F-R9-06 回归锚点：并发写者推进状态后，持过期 row_version 的 publish
        必须得到 VersionConflictError（旧 CAS 对此永远沉默）。"""
        mgr = CapabilityVersionManager()
        v = mgr.register("mcp", "tool")
        stale_row = mgr.row_version(v.id)
        # 并发写者把状态推进到 approved（row_version 随之递增）
        for target in (
            ...
            mgr.transition(v.id, target)
        with pytest.raises(VersionConflictError):
            mgr.transition(v.id, CapabilityStatus.PUBLISHED, expected_version=stale_row)
...
class TestUpstreamCandidate:
    """F-R9-03/T-P3.4：list_changed→candidate——同名已发布版本存在且 content 变化时，
    新注册携带 parent_id + drift 报告；无前驱或内容未变则按全新注册。"""

    def _published(self, mgr: CapabilityVersionManager, digest: str):
        v = mgr.register("mcp", "tool", content_digest=digest)
        for status in (
            CapabilityStatus.QUARANTINED,
            CapabilityStatus.INSPECTED,
            CapabilityStatus.TESTED,
            ...
            v = mgr.transition(v.id, status)
        return v

    def test_content_change_yields_candidate_params(self) -> None:
        from zhiwei.capabilities.versions import prepare_upstream_candidate

        mgr = CapabilityVersionManager()
        parent = self._published(mgr, "sha256:" + "1" * 64)
        params = prepare_upstream_candidate(
            manager=mgr, name="tool", content_digest="sha256:" + "2" * 64
        )
        assert params["parent_id"] == parent.id
        assert params["metadata"]["source"] == "upstream_update"
        assert params["metadata"]["drift"]["passed"] is False

    def test_same_content_yields_no_candidate(self) -> None:
        from zhiwei.capabilities.versions import prepare_upstream_candidate

        mgr = CapabilityVersionManager()
        self._published(mgr, "sha256:" + "1" * 64)
        params = prepare_upstream_candidate(
            manager=mgr, name="tool", content_digest="sha256:" + "1" * 64
        )
        assert params == {}

    def test_no_published_predecessor_yields_no_candidate(self) -> None:
        from zhiwei.capabilities.versions import prepare_upstream_candidate
    ...
    def test_candidate_registers_discovered_with_parent_link(self) -> None:
        from zhiwei.capabilities.versions import prepare_upstream_candidate
        ...
        candidate = mgr.register(
            "mcp",
            "tool",
            content_digest="sha256:" + "2" * 64,
            **params,
        )
        assert candidate.status == CapabilityStatus.DISCOVERED
        assert candidate.parent_id == parent.id
        assert mgr.is_published(parent.id)

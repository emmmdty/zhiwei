...
class TestDelegationChainWiring:
    """F-R2-09：delegation_chain 进 workflow input——命令侧已承载（硬上界在
    RunCommandService），outbox→Temporal 传输段不得丢弃。"""

    @pytest.mark.asyncio
    async def test_start_run_carries_delegation_chain(self) -> None:
        run_id = new_id()
        sender = StubWorkflowSignalSender()
        handler = OutboxSignalHandler(sender=sender)
        payload = _start_run_payload(run_id=run_id)
        payload["delegation_chain"] = ["run-parent", "run-child"]
        delivery = _make_delivery(event_key="start_run", payload=payload)
        await handler.handle(delivery)
        sig = sender.signals[0]
        assert sig["payload"]["delegation_chain"] == ["run-parent", "run-child"]

    @pytest.mark.asyncio
    async def test_start_run_without_chain_defaults_empty(self) -> None:
        run_id = new_id()
        sender = StubWorkflowSignalSender()
        handler = OutboxSignalHandler(sender=sender)
        delivery = _make_delivery(
            event_key="start_run", payload=_start_run_payload(run_id=run_id)
        )
        await handler.handle(delivery)
        sig = sender.signals[0]
        assert sig["payload"]["delegation_chain"] == []

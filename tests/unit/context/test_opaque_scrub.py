...
class TestScrubKeysConfigurable:
    """F-R2-06/T-P3.7：scrub 键名清单可配置——默认集合扩展为四键，显式传参可收窄。"""

    def test_default_keys_cover_extended_set(self) -> None:
        value = {
            "reasoning_content": "body-a",
            "thinking": "body-b",
            "reasoning": "body-c",
            "hidden_reasoning": "body-d",
            "answer": "safe",
        }
        scrubbed = scrub_hidden_reasoning(value)
        assert scrubbed["hidden_reasoning"]["opaque_ref"].startswith("opaque:")
        assert scrubbed["reasoning_content"]["opaque_ref"].startswith("opaque:")
        assert scrubbed["thinking"]["opaque_ref"].startswith("opaque:")
        assert scrubbed["reasoning"]["opaque_ref"].startswith("opaque:")
        assert scrubbed["answer"] == "safe"

    def test_explicit_keys_narrow_the_set(self) -> None:
        value = {"reasoning": "body", "thinking": "body-b"}
        scrubbed = scrub_hidden_reasoning(value, keys=("hidden_reasoning",))
        assert scrubbed["reasoning"] == "body"
        assert scrubbed["thinking"] == "body-b"

    def test_explicit_keys_still_scrub_declared(self) -> None:
        value = {"thinking": "body", "nested": {"thinking": "deep"}}
        scrubbed = scrub_hidden_reasoning(value, keys=("thinking",))
        assert scrubbed["thinking"]["opaque_ref"].startswith("opaque:")
        assert scrubbed["nested"]["thinking"]["opaque_ref"].startswith("opaque:")

    def test_extended_keys_are_idempotent(self) -> None:
        value = {"reasoning": "body", "nested": {"thinking": "deep"}}
        once = scrub_hidden_reasoning(value)
        twice = scrub_hidden_reasoning(once)
        assert twice == once

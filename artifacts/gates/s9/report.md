...
## 2026-09-08 追记（P5/T-P5.3，operator 一次性授权 artifacts 追加，F-R4-02）

§14 权威终轮在干净库上 `eval verify --all-sealed` 到 **checked=14**，而 sealed-runs.json
（本轮权威版）仅 **13 entries**（12 suite/legacy + longmemeval claim）——对账差一的第 14 个
sealed EvalRun 为早期 `seal-empty` plumbing 冒烟轮次产物（verify 全部通过，无效度威胁）；
其身份未逐条入清单系登记遗漏而非数据问题。自本轮起后续 Gate 固定自检断言：
verify 的 checked 数必须等于权威清单 entries 数。本追记为对 sealed-runs.json 不可变记录的
只读澄清，不改动任何 sealed 行。
